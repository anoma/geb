...
PolyExpToDialgFst : (p, q : PolyFunc) ->
  ...
  InterpPolyFunc p a ->
  pfPos q
PolyExpToDialgFst p q x i_d pi_pd =
  fst ((fst i_d) (fst pi_pd))
...
PolyExpToDialgSnd : (p, q : PolyFunc) ->
  ...
  a
PolyExpToDialgSnd p q x i_d pi_pd qdi with
    (snd (fst i_d $ fst pi_pd) qdi) proof eq
  PolyExpToDialgSnd p q x i_d pi_pd qdi
    | Left () =
      snd i_d (fst pi_pd ** (qdi ** rewrite eq in ()))
  PolyExpToDialgSnd p q x i_d pi_pd qdi
    | Right pdi =
      snd pi_pd pdi

export
PolyExpAndPolyInvolutionToDialg : (p, q : PolyFunc) ->
  ((x : Type) -> (x -> x)) ->
  NaturalTransformation
    (InterpPolyFunc (pfHomObj p q))
    (Dialgebra (InterpPolyFunc p) (InterpPolyFunc q))
PolyExpAndPolyInvolutionToDialg p q invol x dialg =
  let
    homNT :
      PolyNatTrans (pfProductArena (PFHomArena x) p) q =
        PolyToInterpHom p q x dialg
    interpDialg :
      InterpPolyFunc (pfProductArena (PFHomArena x) p) x ->
      InterpPolyFunc q x =
        InterpPolyNT {p=(pfProductArena (PFHomArena x) p)} {q} homNT x
  in
  interpDialg . dpBimap (MkPair ()) (\pi, pdmx => eitherElim (invol x) pdmx)
...
export
PolyExpToDialgEqInvolutionVersionFst : (p, q : PolyFunc) ->
  (x : Type) -> (el : InterpPolyFunc (pfHomObj p q) x) ->
  (px : InterpPolyFunc p x) ->
  fst (PolyExpToDialg p q x el px) =
    fst (PolyExpAndPolyInvolutionToDialg p q (\_ => Prelude.id) x el px)
PolyExpToDialgEqInvolutionVersionFst (ppos ** pdir) (qpos ** qdir) x
  el (pi ** pdm) =
    Refl

export
PolyExpToDialgEqInvolutionVersionSnd : (p, q : PolyFunc) ->
  (x : Type) -> (el : InterpPolyFunc (pfHomObj p q) x) ->
  (px : InterpPolyFunc p x) ->
  (qd : pfDir {p=q} $ fst $ fst el $ fst px) ->
  snd (PolyExpToDialg p q x el px) qd =
    snd (PolyExpAndPolyInvolutionToDialg p q (\_ => Prelude.id) x el px) qd
PolyExpToDialgEqInvolutionVersionSnd (ppos ** pdir) (qpos ** qdir) x
    (el1 ** el2) (pi ** pdm) qd with (snd (el1 pi) qd)
  PolyExpToDialgEqInvolutionVersionSnd (ppos ** pdir) (qpos ** qdir) x
    (el1 ** el2) (pi ** pdm) qd | Left () =
      Refl
  PolyExpToDialgEqInvolutionVersionSnd (ppos ** pdir) (qpos ** qdir) x
    (el1 ** el2) (pi ** pdm) qd | Right pd =
      Refl
...
------------------------------------------------------------------
---- Polynomial initial algebras through universal properties ----
------------------------------------------------------------------
...
public export
0 PolyImpredFromToMu : FunExt -> (p : PolyFunc) ->
  (el : PolyFuncMu p) ->
  PolyImpredToMu p (PolyImpredFromMu p el) = el
PolyImpredFromToMu = pfCataInId

public export
0 PolyImpredToFromMu : FunExt -> (p : PolyFunc) ->
  (el : ImpredInitAlg (InterpPolyFunc p)) ->
  (x : Type) -> (alg : Algebra (InterpPolyFunc p) x) ->
  PolyImpredFromMu p (PolyImpredToMu p el) x alg = el x alg
PolyImpredToFromMu fext p el x alg = ?PolyImpredToFromMu_hole

public export
PolyImpredInitAlgIn : (p : PolyFunc) ->
  Algebra (InterpPolyFunc p) (ImpredInitAlg $ InterpPolyFunc p)
PolyImpredInitAlgIn p (i ** dm) =
  PolyImpredFromMu p (InPFM i (PolyImpredToMu p . dm))

public export
PolyImpredInitAlgOut : (p : PolyFunc) ->
  Coalgebra (InterpPolyFunc p) (ImpredInitAlg $ InterpPolyFunc p)
PolyImpredInitAlgOut p alg =
  case PolyImpredToMu p alg of InPFM i dm => (i ** PolyImpredFromMu p . dm)
