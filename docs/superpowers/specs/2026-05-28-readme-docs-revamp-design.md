...
- The committed `docs/common-lisp/manual.md` is a snapshot of the
  generator's current output (replacing the stale relocated copy). It is
  not byte-reproducible: regeneration may rewrite a few cosmetic lines
  (see Non-goals), which is accepted.
...
- Byte-reproducible regeneration is not a goal. The obsolete generator
  renders several elements nondeterministically across image loads
  (default object printers emit heap addresses; mgl-pax autolink
  resolution varies), so `docs/common-lisp/manual.md` is committed as a
  current snapshot rather than a reproducible artifact. Making it
  reproducible would require open-ended changes to the obsolete Common
  Lisp code, which this branch does not undertake.
...
reads as a single conceptual operation: a pure move, a pure deletion, a
skeleton creation, a focused generator code change, or committing
generator output.
...
4. Retarget the generator. In
   ...
   relocated HTML target is covered by a `.gitignore` entry added with
   this change so a generator run leaves no untracked files.

5. Regenerate and commit a snapshot. Run the retargeted generator to
   produce `docs/common-lisp/manual.md`, and commit the regenerated
   bytes, replacing the stale content moved in Change 1. The committed
   file is the generator's current output: it reconciles the pre-existing
   staleness recorded under Current state, but it is a snapshot, not a
   byte-reproducible artifact. Execution established that the obsolete
   generator renders several elements nondeterministically across image
   loads — for example the default printer for the `*one-bit-to-bool*`
   value emits a heap address, and mgl-pax's autolink resolution of
   tokens such as `ALIAS` varies — so a rebuild may rewrite a few
   cosmetic lines. That nondeterminism is accepted, not chased: fixing it
   would mean open-ended changes to the obsolete Common Lisp code, against
   this branch's scope. The HTML emitted alongside is `.gitignore`d
   (Change 4) and not committed. If reproducing the wrapper's rendering
   requires passing `:pages` or a `:source-uri-fn` to `mgl-pax:document`,
   that is part of the Change 4 rewrite, finalized here against observed
   output.
...
The total volume of moved bytes is large; the conceptual change to the
documentation content is nil. Every commit does exactly one conceptual
thing and is individually reviewable as one of: a pure move (optionally
with the single reference update that the move requires), a pure
deletion, a skeleton creation, a focused generator code change
(retargeting output, `.gitignore`), or committing the generator's
output verbatim. A move commit never also alters the moved content; the
one content change to `manual.md` (Change 5) is isolated as committing
generator output and is reviewable as exactly that. Moves precede the
creation of the new root `README.md` so that version control records the
relocation of the old manual as a rename rather than misattributing it
against a freshly created same-named file.
...
- `docs/common-lisp/manual.md` is generator output, not authored here;
  it is not linted or reformatted. It is a committed snapshot of the
  current output, not a byte-reproducible artifact (see Change 5).
...
- The Common Lisp generator is run during execution (Change 5) to
  produce the `manual.md` snapshot; the run is scripted under Roswell as
...
- Making `manual.md` byte-reproducible, if ever wanted: this would
  require both pinning the mgl-pax version (a quicklisp dist pin or
  vendored copy) and removing the generator's nondeterministic renderings
  (object-identity printers, ambiguous autolinks) in the obsolete Common
  Lisp code, plus an automated rebuild-clean check. Out of scope here.
