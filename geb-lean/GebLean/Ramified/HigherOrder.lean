...
* `holeSig`, `holeConstSig` — the two readings of an identifier occurrence in an
  explicit definition's body: the saturated application and the curried
  combinator value.
...
* `appChain_stdConstructorInterp` — the semantic constructor node rule: the
  curried constructor interpretation applied to an argument spine and read at the
  object sort is the free-algebra node `FreeAlg.mk`.
...
denotation of an identifier's constant, and of a curried identifier hole, is
`curryInterp` of the identifier's denotation. Novel packaging. -/
...
hole, of the hole's context as arity and the hole's result sort as result. A
hole stands for an occurrence of a previously defined identifier; this summand
reads that occurrence as a saturated application, taking arguments at the
identifier's context and returning a value at its result sort. Its companion
`holeConstSig` reads the same occurrence as a combinator value at the curried
arrow sort, applied through the application former — the paper's
identifiers-as-combinators reading (Leivant III section 2.3, the higher-order
system, DOI `10.1016/S0168-0072(98)00040-2`). The two readings agree by
`appChain_curryInterp`. Novel packaging. -/
def holeSig (n : Nat) (holeIdx : Fin n → List RType × RType) : SortedSig RType where
  Op := Fin n
  arity j := (holeIdx j).1
  result j := (holeIdx j).2

/-- The curried hole signature of an explicit definition: one nullary operation
per hole, of the hole's curried arrow sort `RType.curried (holeIdx j).1
(holeIdx j).2` as result. This is the combinator reading of an identifier
occurrence — the identifier as a value at its higher type, to be applied through
the application former — dual to `holeSig`'s saturated-application reading (the
paper's identifiers-as-combinators reading, Leivant III section 2.3, the
higher-order system, DOI `10.1016/S0168-0072(98)00040-2`). The two readings
agree by `appChain_curryInterp`. Novel packaging. -/
...
summand, application, the saturated holes for previously defined identifiers,
and their curried-combinator forms (`holeConstSig`). Novel packaging. -/
...
with constructors and application read as usual, each saturated hole read by the
recursive result of the referenced identifier, and each curried hole by the
currying (`curryInterp`) of that recursive result. Novel packaging. -/
...
/-- The semantic constructor node rule (Leivant III section 2.3 with section
2.7): applying the curried constructor interpretation
`curryInterp A (θ^{r_b}) θ (stdConstructorInterp A (⟨θ, hθ⟩, b))` to a full
argument spine `v` and reading the result at the object sort `θ` as a carrier
element yields the free-algebra node `FreeAlg.mk b` on the arguments read as
carrier elements. The application chain inverts the currying
(`appChain_curryInterp`), exposing `stdConstructorInterp`, whose object-sort cast
then unwinds to the node. Novel packaging. -/
theorem appChain_stdConstructorInterp (A : AlgSig) (b : A.B)
    {θ : RType} (hθ : θ.IsObj)
    (v : ∀ i : Fin (List.replicate (A.ar b) θ).length,
      RType.interp (FreeAlg A) ((List.replicate (A.ar b) θ).get i)) :
    cast (RType.interp_isObj (FreeAlg A) hθ)
        (appChain A (List.replicate (A.ar b) θ) θ
          (curryInterp A (List.replicate (A.ar b) θ) θ
            (stdConstructorInterp A (⟨θ, hθ⟩, b))) v)
      = FreeAlg.mk b (fun i =>
          cast
            (by
              rw [show ((List.replicate (A.ar b) θ).get
                    ⟨i.val, by rw [List.length_replicate]; exact i.isLt⟩) = θ from by
                  simp only [List.get_eq_getElem, List.getElem_replicate]]
              exact RType.interp_isObj (FreeAlg A) hθ)
            (v ⟨i.val, by rw [List.length_replicate]; exact i.isLt⟩)) := by
