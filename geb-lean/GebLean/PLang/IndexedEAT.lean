...
/--
FreeAlgEquiv is reflexive (typeclass instance for QuotLiftable).
-/
instance FreeAlgEquiv.instIsRefl (T : IndexedEAT X) (A : Over X) (x : X) :
    IsRefl (PolyFreeM A T.poly x) (FreeAlgEquiv T A x) where
  refl := FreeAlgEquiv.refl x
...
/--
Transport commutes with `Quot.mk`: transporting a quotient element along a
fiber equality is the same as applying `Quot.mk` to the transported
representative.
-/
lemma quot_mk_transport_eq {Y : Type*} {β : Y → Type*}
    {r : ∀ y, β y → β y → Prop} {x y : Y} (h : x = y) (t : β x) :
    h ▸ Quot.mk (r x) t = Quot.mk (r y) (h ▸ t) := by
  subst h
  rfl
...
/--
Applying the quotient map to a representative gives the mapped representative.
-/
@[simp]
lemma freeAlgQuotMapAt_mk (T : IndexedEAT X) (A B : Over X) (g : A ⟶ B) (x : X)
    (t : PolyFreeM A T.poly x) :
    freeAlgQuotMapAt T A B g x (FreeAlgQuot.mk T A t) =
    FreeAlgQuot.mk T B (polyFreeMapAt A B T.poly g x t) := rfl
...
/-! ## Generic Quotient Algebra Construction

For polynomials with finite arities, we can construct the quotient algebra
generically. The structure map lifts through products of quotients using
nested Quot.lift applications.

FreeAlgEquiv.cong ensures the structure map respects equivalence: equivalent
children produce equivalent nodes.  This allows us to define the structure
map on the quotient.
-/

section GenericQuotientAlgebra

variable {X : Type u}

/-! ### Helper: Extract children from polynomial evaluation -/

/--
Given a morphism from a family to the quotient carrier, extract the quotient
element at each child position by transporting along the fiber equality.
-/
def extractQuotChildren (T : IndexedEAT X) (A : Over X) (x : X)
    (p : polyBetweenIndex X X T.poly x)
    (childMor : polyBetweenFamily X X T.poly x p ⟶ freeAlgQuotOver T A) :
    ∀ e : (polyBetweenFamily X X T.poly x p).left,
      FreeAlgQuot T A ((polyBetweenFamily X X T.poly x p).hom e) := by
  intro e
  let child := childMor.left e
  have fiber_eq : child.fst = (polyBetweenFamily X X T.poly x p).hom e :=
    congrFun (Over.w childMor) e
  exact fiber_eq ▸ child.snd

/-! ### Generic structure map via representative lifting

The structure map on the quotient is defined by:
1. For each position p with arity family F
2. Given children as quotient elements
3. Lift through the quotients to form the node and quotient it

The lifting uses the FreeAlgEquiv.cong congruence property.
-/

/--
The generic structure map on representatives.
Given a position p and representative children, form the node and quotient it.
-/
def genericQuotStrRepr (T : IndexedEAT X) (A : Over X) (x : X)
    (p : polyBetweenIndex X X T.poly x)
    (reps : ∀ e : (polyBetweenFamily X X T.poly x p).left,
      PolyFreeM A T.poly ((polyBetweenFamily X X T.poly x p).hom e)) :
    FreeAlgQuot T A x :=
  freeAlgQuotStrRepr T A x p reps

/--
The structure map on representatives respects pointwise equivalence.
This is the congruence property that allows lifting through quotients.
-/
lemma genericQuotStrRepr_resp (T : IndexedEAT X) (A : Over X) (x : X)
    (p : polyBetweenIndex X X T.poly x)
    (c1 c2 : ∀ e : (polyBetweenFamily X X T.poly x p).left,
      PolyFreeM A T.poly ((polyBetweenFamily X X T.poly x p).hom e))
    (h : ∀ e, FreeAlgEquiv T A _ (c1 e) (c2 e)) :
    genericQuotStrRepr T A x p c1 = genericQuotStrRepr T A x p c2 :=
  freeAlgQuotStrRepr_resp T A x p c1 c2 h

/-! ### Lifting through quotients for specific arities

For practical use, we provide lifting combinators for common arity types.
These are used directly by clients to define their quotient structure maps.
-/

/--
Lift a function through a single quotient (arity = Unit).
-/
def quotLiftUnit {β : Type u} (r : β → β → Prop) {γ : Type u}
    (f : β → γ) (hf : ∀ b₁ b₂, r b₁ b₂ → f b₁ = f b₂) (q : Quot r) : γ :=
  Quot.lift f hf q

/--
Helper for quotLiftBool: construct a product function from Bool components.
-/
def boolProd {β : Bool → Type u} (t : β true) (f : β false) : ∀ b, β b
  | true => t
  | false => f

/--
Lift a function through two quotients (arity = Bool).
Requires IsRefl on each component relation.
-/
def quotLiftBool {β : Bool → Type u} (r : ∀ b, β b → β b → Prop)
    [h_true : IsRefl (β true) (r true)] [h_false : IsRefl (β false) (r false)]
    {γ : Type u}
    (f : (∀ b, β b) → γ)
    (hf : ∀ g₁ g₂, (∀ b, r b (g₁ b) (g₂ b)) → f g₁ = f g₂)
    (qs : ∀ b, Quot (r b)) : γ :=
  let mkPairResp (rt : β true) : ∀ rf₁ rf₂, r false rf₁ rf₂ →
      f (boolProd rt rf₁) = f (boolProd rt rf₂) :=
    fun _ _ hrel => hf _ _ fun b =>
      match b with | true => h_true.refl rt | false => hrel
  let liftFalse (rt : β true) : γ :=
    Quot.lift (f ∘ boolProd rt) (mkPairResp rt) (qs false)
  let liftFalseResp (rt₁ rt₂ : β true) (hrel : r true rt₁ rt₂) :
      liftFalse rt₁ = liftFalse rt₂ := by
    simp only [liftFalse]
    apply @Quot.ind (β false) (r false) (fun q =>
      Quot.lift (f ∘ boolProd rt₁) (mkPairResp rt₁) q =
      Quot.lift (f ∘ boolProd rt₂) (mkPairResp rt₂) q)
    intro rf
    simp only [Function.comp_apply]
    exact hf _ _ fun b =>
      match b with | true => hrel | false => h_false.refl rf
  Quot.lift liftFalse liftFalseResp (qs true)

/--
The structure map on representatives respects pointwise equivalence.
This is the congruence property that allows lifting through quotients.

Clients can use this with the specific lifting combinators above to define
their quotient structure maps.
-/
lemma genericQuotStrRepr_resp' (T : IndexedEAT X) (A : Over X) (x : X)
    (p : polyBetweenIndex X X T.poly x)
    (c1 c2 : ∀ e : (polyBetweenFamily X X T.poly x p).left,
      PolyFreeM A T.poly ((polyBetweenFamily X X T.poly x p).hom e))
    (h : ∀ e, FreeAlgEquiv T A _ (c1 e) (c2 e)) :
    freeAlgQuotStrRepr T A x p c1 = freeAlgQuotStrRepr T A x p c2 :=
  freeAlgQuotStrRepr_resp T A x p c1 c2 h

/-! ### Typeclass for quotient-liftable arity types

An arity type E is quotient-liftable if we can lift a function on product of
representatives through a product of quotients. This captures the ability to
define structure maps on quotients generically.
-/

/--
A type `E` is quotient-liftable if we can lift functions through products of
quotients indexed by `E`. This is used to define the quotient algebra structure
generically for polynomials with liftable arities.
-/
class QuotLiftable (E : Type u) where
  /-- Lift a function on products of representatives through products of quotients.
      - `Y : E → Type u` - the type family at each position
      - `r : ∀ e, Y e → Y e → Prop` - the relation at each position
      - `Z : Type u` - the result type
      - `f : (∀ e, Y e) → Z` - the function on representatives
      - `hf` - proof that `f` respects pointwise relatedness
      - `qs : ∀ e, Quot (r e)` - the quotient inputs
  -/
  quotLiftPi :
    {Y : E → Type u} →
    (r : ∀ e, Y e → Y e → Prop) →
    ...
    {Z : Type u} →
    (f : (∀ e, Y e) → Z) →
    (hf : ∀ g₁ g₂, (∀ e, r e (g₁ e) (g₂ e)) → f g₁ = f g₂) →
    (qs : ∀ e, Quot (r e)) → Z
  /-- When applied to representatives, quotLiftPi equals the function directly. -/
  quotLiftPi_mk :
    {Y : E → Type u} →
    (r : ∀ e, Y e → Y e → Prop) →
    [∀ e, IsRefl (Y e) (r e)] →
    ...
    (hf : ∀ g₁ g₂, (∀ e, r e (g₁ e) (g₂ e)) → f g₁ = f g₂) →
    (reps : ∀ e, Y e) →
    quotLiftPi r f hf (fun e => Quot.mk (r e) (reps e)) = f reps

/-- Unit is trivially quotient-liftable: just lift through the single quotient. -/
instance : QuotLiftable Unit where
  quotLiftPi := fun {Y} _ _ {_} f hf qs =>
    Quot.lift
      (fun (y : Y ()) => f (fun (_ : Unit) => y))
      (fun _ _ hr => hf _ _ (fun (_ : Unit) => hr))
      (qs ())
  quotLiftPi_mk := fun {_} _ _ {_} f _ _ =>
    congrArg f (funext fun u => match u with | () => rfl)

/-- Bool is quotient-liftable via nested lifting. -/
instance : QuotLiftable Bool where
  quotLiftPi := fun {_} r _ {_} f hf qs =>
    quotLiftBool r f hf qs
  quotLiftPi_mk := fun {_} _ _ {_} f _ _ =>
    congrArg f (funext fun b => match b with | true => rfl | false => rfl)

/-! ### Generic Quotient Structure Map

Given an EAT T with a position p whose arity type is QuotLiftable, we can
define the structure map on quotients by lifting through the product of
quotients indexed by the arity type.
-/

/--
A P-algebra node in the free monad with given position and children.
This creates a PolyFreeM node from position p and children.
-/
def polyFreeMNode (A : Over X) (P : PolyEndo X) (x : X)
    (p : polyBetweenIndex X X P x)
    (children : ∀ e : (polyBetweenFamily X X P x p).left,
      PolyFreeM A P ((polyBetweenFamily X X P x p).hom e)) :
    PolyFreeM A P x :=
  PolyFix.mk x (Sum.inr p) children

/--
The generic quotient structure map at a specific fiber and position.

Given quotient children (elements of FreeAlgQuot), lifts through the product
of quotients to produce a quotient element. This uses:
- `QuotLiftable.quotLiftPi` to lift through the product
- `FreeAlgEquiv.cong` to prove well-definedness
-/
def genericQuotientStrAt (T : IndexedEAT X) (A : Over X) (x : X)
    (p : polyBetweenIndex X X T.poly x)
    [inst : QuotLiftable ((polyBetweenFamily X X T.poly x p).left)]
    (children : ∀ e : (polyBetweenFamily X X T.poly x p).left,
      FreeAlgQuot T A ((polyBetweenFamily X X T.poly x p).hom e)) :
    FreeAlgQuot T A x :=
  let E := (polyBetweenFamily X X T.poly x p).left
  let fam := polyBetweenFamily X X T.poly x p
  @QuotLiftable.quotLiftPi E inst
    (fun e => PolyFreeM A T.poly (fam.hom e))
    (fun e => FreeAlgEquiv T A (fam.hom e))
    (fun _ => FreeAlgEquiv.instIsRefl T A _)
    (FreeAlgQuot T A x)
    (fun reps => FreeAlgQuot.mk T A (polyFreeMNode A T.poly x p reps))
    (fun g₁ g₂ h => Quot.sound (FreeAlgEquiv.cong x p g₁ g₂ h))
    children

/--
When genericQuotientStrAt is applied to representative children (via mk),
it equals the quotient of the node formed from those representatives.
-/
lemma genericQuotientStrAt_mk (T : IndexedEAT X) (A : Over X) (x : X)
    (p : polyBetweenIndex X X T.poly x)
    [inst : QuotLiftable ((polyBetweenFamily X X T.poly x p).left)]
    (reps : ∀ e : (polyBetweenFamily X X T.poly x p).left,
      PolyFreeM A T.poly ((polyBetweenFamily X X T.poly x p).hom e)) :
    genericQuotientStrAt T A x p (fun e => FreeAlgQuot.mk T A (reps e)) =
    FreeAlgQuot.mk T A (polyFreeMNode A T.poly x p reps) := by
  let E := (polyBetweenFamily X X T.poly x p).left
  let fam := polyBetweenFamily X X T.poly x p
  simp only [genericQuotientStrAt, FreeAlgQuot.mk]
  exact @QuotLiftable.quotLiftPi_mk E inst
    (fun e => PolyFreeM A T.poly (fam.hom e))
    (fun e => FreeAlgEquiv T A (fam.hom e))
    (fun _ => FreeAlgEquiv.instIsRefl T A _)
    (Quot (FreeAlgEquiv T A x))
    (fun reps' => Quot.mk _ (polyFreeMNode A T.poly x p reps'))
    (fun g₁ g₂ h => Quot.sound (FreeAlgEquiv.cong x p g₁ g₂ h))
    reps

/-! ### Polynomials with liftable arities

For the generic quotient algebra construction, we need all positions of the
polynomial to have QuotLiftable arity types. This class captures this constraint.
-/

/--
A polynomial P has liftable arities if for every fiber x and every position p,
the arity type `(polyBetweenFamily X X P x p).left` is QuotLiftable.
-/
class PolyHasLiftableArities (P : PolyEndo X) where
  /-- Each position has a QuotLiftable arity type. -/
  liftableAt : ∀ (x : X) (p : polyBetweenIndex X X P x),
    QuotLiftable ((polyBetweenFamily X X P x p).left)

attribute [instance] PolyHasLiftableArities.liftableAt

/--
The full generic quotient structure map left component.
Given an element of P(FreeAlgQuot), produces an element of FreeAlgQuot.
-/
def genericQuotientStrLeft (T : IndexedEAT X) [PolyHasLiftableArities T.poly]
    (A : Over X) :
    ((polyEndoFunctor X T.poly).obj (freeAlgQuotOver T A)).left →
    (freeAlgQuotOver T A).left :=
  fun ⟨x, elem⟩ =>
    let p := ccrEvalIndex elem
    let childMor := ccrEvalMor elem
    let extractChild (e : (polyBetweenFamily X X T.poly x p).left) :
        FreeAlgQuot T A ((polyBetweenFamily X X T.poly x p).hom e) :=
      let child := childMor.left e
      have fiber_eq : child.1 = (polyBetweenFamily X X T.poly x p).hom e :=
        congrFun (Over.w childMor) e
      fiber_eq ▸ child.2
    ⟨x, genericQuotientStrAt T A x p extractChild⟩

/--
The generic quotient structure map commutes with fiber projections.
-/
lemma genericQuotientStr_comm (T : IndexedEAT X) [PolyHasLiftableArities T.poly]
    (A : Over X) :
    genericQuotientStrLeft T A ≫ (freeAlgQuotOver T A).hom =
    ((polyEndoFunctor X T.poly).obj (freeAlgQuotOver T A)).hom := rfl

/--
The generic quotient structure map as an Over morphism.
-/
def genericQuotientStr (T : IndexedEAT X) [PolyHasLiftableArities T.poly]
    (A : Over X) :
    (polyEndoFunctor X T.poly).obj (freeAlgQuotOver T A) ⟶ freeAlgQuotOver T A :=
  Over.homMk (genericQuotientStrLeft T A) (genericQuotientStr_comm T A)

/--
The generic quotient P-algebra for an EAT with liftable arities.
-/
def genericQuotientAlg (T : IndexedEAT X) [PolyHasLiftableArities T.poly]
    (A : Over X) : PolyAlg T.poly where
  a := freeAlgQuotOver T A
  str := genericQuotientStr T A

/--
Extract quotient children from a morphism to the quotient carrier.
Given mor : family → freeAlgQuotOver, extracts the quotient element at each
child position.
-/
def genericExtractQuotChildren (T : IndexedEAT X) (A : Over X) (x : X)
    (p : polyBetweenIndex X X T.poly x)
    (mor : polyBetweenFamily X X T.poly x p ⟶ freeAlgQuotOver T A) :
    ∀ e : (polyBetweenFamily X X T.poly x p).left,
      FreeAlgQuot T A ((polyBetweenFamily X X T.poly x p).hom e) :=
  fun e =>
    let child := mor.left e
    have fiber_eq : child.1 = (polyBetweenFamily X X T.poly x p).hom e :=
      congrFun (Over.w mor) e
    fiber_eq ▸ child.2

/--
When composing a morphism to the free algebra with the quotient map, extracting
children gives `FreeAlgQuot.mk` applied to the transported original children.
-/
lemma genericExtractQuotChildren_comp_quotient (T : IndexedEAT X) (A : Over X)
    (x : X) (p : polyBetweenIndex X X T.poly x)
    (mor : polyBetweenFamily X X T.poly x p ⟶ polyFreeMCarrier A T.poly)
    (e : (polyBetweenFamily X X T.poly x p).left) :
    genericExtractQuotChildren T A x p (mor ≫ freeAlgQuotMap T A) e =
    FreeAlgQuot.mk T A (congrFun (Over.w mor) e ▸ (mor.left e).snd) := by
  simp only [genericExtractQuotChildren, freeAlgQuotMap, CategoryTheory.Over.comp_left,
    Over.homMk_left, types_comp_apply]
  exact quot_mk_transport_eq _ _

/--
The quotient map from the free algebra to the generic quotient algebra.
This is an algebra homomorphism: the structure map on the free algebra followed
by quotient equals quotient followed by structure map on the quotient.
-/
def genericQuotientHom (T : IndexedEAT X) [PolyHasLiftableArities T.poly]
    (A : Over X) : polyFreeAlg A T.poly ⟶ genericQuotientAlg T A where
  f := freeAlgQuotMap T A
  h := by
    apply Over.OverMorphism.ext
    funext ⟨x, elem⟩
    obtain ⟨p, mor⟩ := elem
    simp only [Over.comp_left, types_comp_apply]
    simp only [genericQuotientAlg, genericQuotientStr, Over.homMk_left,
      genericQuotientStrLeft]
    ...
    simp only [polyEndoFunctor, polyBetweenEvalFunctor, polyToOverFunctor,
      polyToOverEvalMap] at *
    simp only [familySliceForward, familySliceForwardMap,
      polyToOverEvalFamilyMap, ccrEvalMap, ccrEvalIndex, ccrEvalMor] at *
    simp only [freeAlgQuotMap, Over.homMk_left]
    apply congrArg (Sigma.mk _)
    simp only [polyFreeMStrFamily]
    have h_children : ∀ e, genericExtractQuotChildren T A x p
        (mor ≫ freeAlgQuotMap T A) e =
        FreeAlgQuot.mk T A (congrFun (Over.w mor) e ▸ (mor.left e).snd) :=
      fun e => genericExtractQuotChildren_comp_quotient T A x p mor e
    simp only [genericExtractQuotChildren, freeAlgQuotMap, Over.comp_left,
      Over.homMk_left, types_comp_apply] at h_children ⊢
    conv_lhs =>
      rw [funext h_children]
    rw [genericQuotientStrAt_mk]
    simp only [polyFreeMNode, FreeAlgQuot.mk, pbefIndex, pbefMor, ptoefIndex, ptoefMor,
      ccrEvalIndex, ccrEvalMor]
    rfl

end GenericQuotientAlgebra
