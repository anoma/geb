...
`bsum_prologueSrc k s`, destination register `(k + 10) +
(frag_f.inputRegs s).val`, scratch register `k + 5` (`tmp1`), and
reserved-zero register `0`. Mirrors
`compileFrag_bsum_prologueBlock_instr_at`, specialised to bprod's
per-iteration prologue (which reuses the same `bsum_prologueBlock`
template but at a different `prologueBase` and `fBase`). -/
private theorem compileFrag_bprod_prologueBlock_instr_at
    {k : ℕ}
    (frag_f : CompiledFragment (k + 1))
    (s : Fin (k + 1)) :
    let outer := (compileFrag_bprod frag_f).toURMProgram
    ∃ (h_src : bsum_prologueSrc k s < outer.numRegs)
      ...
      preservingTransferInstrs outer (bprod_prologueBase frag_f + 9 * s.val)
        ...
        ⟨0, h_z⟩ := by
  intro outer
  -- Abbreviations mirroring the constructor of `compileFrag_bprod`.
  let vAcc : ℕ := 1
  let vBoundIn : ℕ := 2
  let vX : ℕ := k + 3
  let vI : ℕ := k + 4
  ...
  have h_src_lt : bsum_prologueSrc k s < outer.numRegs := by
    ...
    split <;> omega
  have h_dst_lt : (k + 10) + (frag_f.inputRegs s).val < outer.numRegs := by
    have hI : (frag_f.inputRegs s).val < frag_f.numRegs :=
      (frag_f.inputRegs s).isLt
    rw [h_numRegs_eq]
    change (k + 10) + (frag_f.inputRegs s).val < fBase + frag_f.numRegs
    omega
  have h_tmp_lt : k + 5 < outer.numRegs := by
    rw [h_numRegs_eq]
    have h_pos : 0 < frag_f.numRegs := frag_f.numRegs_pos
    change k + 5 < fBase + frag_f.numRegs; omega
  have h_z_lt : 0 < outer.numRegs := (compileFrag_bprod frag_f).numRegs_pos
  ...
  have h_outerInstr_lookup :
    ...
    have h_idx_lt_outer :
      ...
      change 16 + frag_f.numRegs + 9 * s.val + d < _
      have hs : s.val < k + 1 := s.isLt
      omega
...
(PC = `bprod_bodyPCBase frag_f`, the start of f's body).
Strengthens `compileFrag_bprod_phase_i0_post` by recording that f's
...
`vX_eq`, `vI_eq`, `tmp1_zero`, `tmp2_zero`, `accClone_zero`,
`factor_zero`, `mulTmp_zero`, `acc_eq`) carry over verbatim from
`compileFrag_bprod_phase_i0_post @ i`. The `f_other_zero` clause
records that f's reindexed block outside the now-filled input slots
remains 0 (preserved from phase_i0's `fBody_zero` since the prologue's
preservingTransfer touches only the input-slot destinations and
`tmp1`). -/
private structure compileFrag_bprod_phase_i1_post
  ...
  f_inputs : ∀ (j : Fin (k + 1)),
    s.regs ⟨(k + 10) + (frag_f.inputRegs j).val, by
      ...
      change (k + 10) + (frag_f.inputRegs j).val
        < k + 10 + frag_f.numRegs
...
set_option maxHeartbeats 4000000 in
-- The 14-field discharge of `compileFrag_bprod_phase_i1_post` through
-- `compileFrag_bprod_prologue_correct` exceeds the default heartbeat
-- budget on the bprod numRegs profile (three extra mul-scratch fields
-- relative to bsum).
/-- Phase i.1 preservation lemma: from
`compileFrag_bprod_phase_i0_post @ i`, running for `T0 :=
9 * vPrefixSum (Fin.cons i.val (Fin.tail v)) (k + 1) + 2 * (k + 1)`
further steps (the per-iteration prologue) lands the state in
`compileFrag_bprod_phase_i1_post @ i`. The accompanying strict PC
bound states that during these `T0` steps the intermediate PC stays
strictly less than `bprod_bodyPCBase frag_f`. -/
private theorem compileFrag_bprod_partial_phase_i1
    ...
    (sPre : URMState
      (compileFrag_bprod (compileERFrag f)).toURMProgram)
    (h_i0 : compileFrag_bprod_phase_i0_post (compileERFrag f) f v i sPre) :
    let T0 : ℕ := 9 * vPrefixSum (Fin.cons i.val (Fin.tail v)) (k + 1)
      + 2 * (k + 1)
    compileFrag_bprod_phase_i1_post (compileERFrag f) f v i
        (URMState.runFor
          (compileFrag_bprod (compileERFrag f)).toURMProgram sPre T0)
      ∧ (∀ k' < T0,
        (URMState.runFor (compileFrag_bprod (compileERFrag f)).toURMProgram
          sPre k').pc
          < bprod_bodyPCBase (compileERFrag f)) := by
  let frag_f : CompiledFragment (k + 1) := compileERFrag f
  let outerFrag : CompiledFragment (k + 1) := compileFrag_bprod frag_f
  let P : URMProgram (k + 1) := outerFrag.toURMProgram
  let v_iter : Fin (k + 1) → ℕ := Fin.cons i.val (Fin.tail v)
  let pcBase : ℕ := bprod_prologueBase frag_f
  change
    compileFrag_bprod_phase_i1_post frag_f f v i
        (URMState.runFor P sPre
          (9 * vPrefixSum v_iter (k + 1) + 2 * (k + 1)))
      ∧ (∀ k' < 9 * vPrefixSum v_iter (k + 1) + 2 * (k + 1),
        (URMState.runFor P sPre k').pc < bprod_bodyPCBase frag_f)
  have h_numRegs_eq : P.numRegs = (k + 10) + frag_f.numRegs := rfl
  have h_numRegs_pos : 0 < P.numRegs := outerFrag.numRegs_pos
  -- Bound proofs.
  ...
  have h_dst_lt : ∀ (j : Fin (k + 1)),
      (k + 10) + (frag_f.inputRegs j).val < P.numRegs := by
    intro j
    have hI : (frag_f.inputRegs j).val < frag_f.numRegs :=
      (frag_f.inputRegs j).isLt
    rw [h_numRegs_eq]
  ...
  have h_disj : inputCopies_disj P zReg tmpFin srcs dsts := by
    ...
    · intro j hh
      have h_val : (0 : ℕ) = (k + 10) + (frag_f.inputRegs j).val :=
        congrArg Fin.val hh
      omega
    · intro hh
      ...
      omega
    · intro j hh
      have h_val : bsum_prologueSrc k j = (k + 10) + (frag_f.inputRegs j).val :=
        congrArg Fin.val hh
      simp only [bsum_prologueSrc] at h_val
      have hI : (frag_f.inputRegs j).val < frag_f.numRegs :=
        (frag_f.inputRegs j).isLt
    ...
    · intro j hh
      ...
      split at h_val <;> omega
    · intro j hh
      have h_val : (k + 10) + (frag_f.inputRegs j).val = k + 5 :=
        congrArg Fin.val hh
      omega
    · intro j₁ j₂ hne hh
  ...
  have h_H : ∀ (j : Fin (k + 1)),
    ...
    intro j
    obtain ⟨_, _, _, _, hPT⟩ :=
      compileFrag_bprod_prologueBlock_instr_at frag_f j
    exact hPT
  -- Pre-state hypotheses from `compileFrag_bprod_phase_i0_post`.
  ...
  obtain ⟨h_pc_post, h_z_post, h_tmp_post, h_dsts_post,
    ...
    compileFrag_bprod_prologue_correct frag_f zReg tmpFin srcs dsts
      h_disj h_H v_iter sPre h_s_pc h_s_z h_s_tmp h_s_srcs h_s_dsts
  refine ⟨?_, ?_⟩
  · -- Discharge each `compileFrag_bprod_phase_i1_post` clause.
    refine
      { hi_lt := i.isLt
        pc_eq := ?_
        zReg_zero := ?_
        ...
        f_other_zero := ?_ }
    · -- pc_eq.
      change (URMState.runFor P sPre _).pc = bprod_bodyPCBase frag_f
      rw [h_pc_post]
      rfl
    · -- zReg_zero.
      have h_idx_eq : (⟨0, (compileFrag_bprod frag_f).numRegs_pos⟩
    ...
    · -- outer_inputs preserved via h_oth_post.
      ...
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : 2 + j.val = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        have hj : j.val < k + 1 := j.isLt
        omega
    ...
    · -- vX_eq.
      ...
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : k + 3 = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        omega
      have h_ne_tmp : r ≠ tmpFin := by
        intro hh
    ...
    · -- vI_eq.
      ...
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : k + 4 = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        omega
      have h_ne_tmp : r ≠ tmpFin := by
        intro hh
    ...
    · -- tmp2_zero preserved via h_oth_post.
      ...
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : k + 6 = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        omega
      have h_ne_tmp : r ≠ tmpFin := by
        intro hh
    ...
    · -- accClone_zero preserved via h_oth_post.
      let r : Fin P.numRegs := ⟨k + 7, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : k + 7 = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        omega
      have h_ne_tmp : r ≠ tmpFin := by
        intro hh
        have h_val : k + 7 = k + 5 := congrArg Fin.val hh
        omega
      have h_oth := h_oth_post r h_ne_dsts h_ne_tmp
      have h_idx_eq :
          (⟨k + 7, by
            ...
            omega⟩ : Fin P.numRegs) = r := Fin.ext rfl
      rw [h_idx_eq, h_oth]
      exact h_i0.accClone_zero
    · -- factor_zero preserved via h_oth_post.
      let r : Fin P.numRegs := ⟨k + 8, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : k + 8 = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        omega
      have h_ne_tmp : r ≠ tmpFin := by
        intro hh
        have h_val : k + 8 = k + 5 := congrArg Fin.val hh
        omega
      have h_oth := h_oth_post r h_ne_dsts h_ne_tmp
      have h_idx_eq :
          (⟨k + 8, by
            ...
            omega⟩ : Fin P.numRegs) = r := Fin.ext rfl
      rw [h_idx_eq, h_oth]
      exact h_i0.factor_zero
    · -- mulTmp_zero preserved via h_oth_post.
      let r : Fin P.numRegs := ⟨k + 9, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : k + 9 = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        omega
      have h_ne_tmp : r ≠ tmpFin := by
        intro hh
        have h_val : k + 9 = k + 5 := congrArg Fin.val hh
        omega
      have h_oth := h_oth_post r h_ne_dsts h_ne_tmp
      have h_idx_eq :
          (⟨k + 9, by
    ...
    · -- acc_eq preserved via h_oth_post.
      ...
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), r ≠ dsts j' := by
        intro j' hh
        have h_val : (1 : ℕ) = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
        omega
      have h_ne_tmp : r ≠ tmpFin := by
        intro hh
    ...
    · -- f_inputs: dsts j filled with v_iter j.
      intro j
      have h_idx_eq :
          (⟨(k + 10) + (frag_f.inputRegs j).val, by
            have hI : (frag_f.inputRegs j).val < frag_f.numRegs :=
              (frag_f.inputRegs j).isLt
            change (k + 10) + (frag_f.inputRegs j).val
              < k + 10 + frag_f.numRegs
    ...
    · -- f_other_zero: r outside the filled input slots stays at 0
      -- via h_oth_post applied to (k+10)+r.val.
      intro r h_not_input
      let R : Fin P.numRegs := ⟨(k + 10) + r.val, by
        have hr : r.val < frag_f.numRegs := r.isLt
        rw [h_numRegs_eq]; omega⟩
      have h_ne_dsts : ∀ (j' : Fin (k + 1)), R ≠ dsts j' := by
        intro j' hh
        have h_val : (k + 10) + r.val
            = (k + 10) + (frag_f.inputRegs j').val :=
          congrArg Fin.val hh
      ...
      have h_ne_tmp : R ≠ tmpFin := by
        intro hh
        have h_val : (k + 10) + r.val = k + 5 := congrArg Fin.val hh
        omega
      have h_oth := h_oth_post R h_ne_dsts h_ne_tmp
      have h_idx_eq :
          (⟨(k + 10) + r.val, by
  ...
  · -- Strict PC bound during the T0 prologue steps.
    intro k' hk'
    have h_bound :=
      compileFrag_bprod_prologue_pc_strict_bound frag_f zReg tmpFin
        srcs dsts h_disj h_H v_iter sPre h_s_pc h_s_z h_s_tmp
        h_s_srcs h_s_dsts k' hk'
    change (URMState.runFor P sPre k').pc < bprod_bodyPCBase frag_f
...
/-- Phase i.2 post-state for `compileFrag_bprod`'s inner iteration:
after f's reindexed body has run to completion, the PC sits at
`bprod_trBase frag_f`, the first instruction of the accumulator-update
block. The `f_output` clause records that f's reindexed output register
holds `f.interp (Fin.cons i.val (Fin.tail v))`. All outer-scaffolding
clauses (`hi_lt`, `zReg_zero`, `outer_inputs`, `vX_eq`, `vI_eq`,
`tmp1_zero`, `tmp2_zero`, `accClone_zero`, `factor_zero`, `mulTmp_zero`,
`acc_eq`) carry over verbatim from `compileFrag_bprod_phase_i1_post @ i`,
since every outer-scaffolding register lives at an index `< k + 10`
outside f's reindexed block `[k + 10, k + 10 + frag_f.numRegs)`.
The `f_inputs` and `f_other_zero` clauses are dropped: after f's body
runs, only f's output register is constrained, and the rest of f's
block may hold arbitrary intermediate values. -/
...
set_option maxHeartbeats 4000000 in
-- The 12-field discharge of `compileFrag_bprod_phase_i2_post` plus the
-- state-embedding plumbing exceeds the default heartbeat budget on the
-- bprod numRegs profile (three extra mul-scratch fields relative to
-- bsum).
/-- Phase i.2 preservation lemma: from
`compileFrag_bprod_phase_i1_post @ i` plus the structural IH on `f`
instantiated at the iteration input vector `Fin.cons i.val (Fin.tail v)`,
there exists some `T0 ≤ compileER_runtime f (Fin.cons i.val (Fin.tail v))`
such that running f's reindexed body for `T0` steps lands the state in
...
`stateEmbedsFrag_runFor`, and the outer scaffolding's preservation
through `stateEmbedsFrag_runFor_outside_preserved`. -/
private theorem compileFrag_bprod_partial_phase_i2
    ...
    (sPre : URMState
      (compileFrag_bprod (compileERFrag f)).toURMProgram)
    (h_i1 : compileFrag_bprod_phase_i1_post (compileERFrag f) f v i sPre) :
    ∃ T0 : ℕ,
      ...
      ∧ (∀ k' < T0,
        (URMState.runFor
          (compileFrag_bprod (compileERFrag f)).toURMProgram sPre k').pc
          < bprod_trBase (compileERFrag f)) := by
  let frag_f : CompiledFragment (k + 1) := compileERFrag f
  let outerFrag : CompiledFragment (k + 1) := compileFrag_bprod frag_f
  let P : URMProgram (k + 1) := outerFrag.toURMProgram
  let v_iter : Fin (k + 1) → ℕ := Fin.cons i.val (Fin.tail v)
  let fBase : ℕ := k + 10
  let pcBase : ℕ := 16 + frag_f.numRegs + 9 * (k + 1)
  ...
  have h_pc_pre : sPre.pc = pcBase := by
    have h := h_i1.pc_eq
    change sPre.pc = bprod_bodyPCBase frag_f at h
    change sPre.pc = 16 + frag_f.numRegs + 9 * (k + 1)
    exact h
  -- Pre-state registers inside f's block match
  -- `(URMState.init frag_f.toURMProgram v_iter).regs`.
  -- `URMState.init` is inlined (no `let`) so its regs projection
  -- reduces during the case split on `List.finRange.find?`.
  have h_pre_regs_in :
      ∀ (r : Fin frag_f.numRegs),
  ...
  · -- Discharge each `compileFrag_bprod_phase_i2_post` clause.
    refine
      { hi_lt := i.isLt
        pc_eq := ?_
        zReg_zero := ?_
        ...
        acc_eq := ?_
        f_output := ?_ }
    · -- pc_eq: PC after T0 = pcBase + (frag_f.instrs.size - 1) = bprod_trBase.
      change (URMState.runFor P sPre T0).pc = bprod_trBase frag_f
      rw [h_pc_after, h_pc_inner]
      change pcBase + (frag_f.instrs.size - 1)
        = bprod_bodyPCBase frag_f + (frag_f.instrs.size - 1)
      rfl
    · -- zReg_zero: register 0 is outside f's block (0 < k + 10).
      let z : Fin P.numRegs := ⟨0, h_numRegs_pos⟩
      have h_out : z.val < fBase ∨ fBase + frag_f.numRegs ≤ z.val := by
        left; change 0 < k + 10; omega
      have h_pres := h_outside_preserved z h_out
      have h_idx_eq :
          (⟨0, (compileFrag_bprod frag_f).numRegs_pos⟩ : Fin P.numRegs)
            = z := Fin.ext rfl
      rw [h_idx_eq, h_pres]; exact h_i1.zReg_zero
    · -- outer_inputs: each `2 + j.val < k + 10` (since j.val < k + 1).
      intro j
      have hj : j.val < k + 1 := j.isLt
      let r : Fin P.numRegs := ⟨2 + j.val, by
        rw [h_numRegs_eq]
      ...
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change 2 + j.val < k + 10; omega
    ...
    · -- vX_eq: register `k + 3 < k + 10`.
      let r : Fin P.numRegs := ⟨k + 3, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change k + 3 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨k + 3, by
            change k + 3 < k + 10 + frag_f.numRegs
    ...
    · -- vI_eq: register `k + 4 < k + 10`.
      let r : Fin P.numRegs := ⟨k + 4, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change k + 4 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨k + 4, by
            change k + 4 < k + 10 + frag_f.numRegs
    ...
    · -- tmp1_zero: register `k + 5 < k + 10`.
      let r : Fin P.numRegs := ⟨k + 5, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change k + 5 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨k + 5, by
            change k + 5 < k + 10 + frag_f.numRegs
    ...
    · -- tmp2_zero: register `k + 6 < k + 10`.
      let r : Fin P.numRegs := ⟨k + 6, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change k + 6 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨k + 6, by
            change k + 6 < k + 10 + frag_f.numRegs
    ...
    · -- accClone_zero: register `k + 7 < k + 10`.
      let r : Fin P.numRegs := ⟨k + 7, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change k + 7 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨k + 7, by
            change k + 7 < k + 10 + frag_f.numRegs
            have : 0 < frag_f.numRegs := frag_f.numRegs_pos
            omega⟩ : Fin P.numRegs) = r := Fin.ext rfl
      rw [h_idx_eq, h_pres]; exact h_i1.accClone_zero
    · -- factor_zero: register `k + 8 < k + 10`.
      let r : Fin P.numRegs := ⟨k + 8, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change k + 8 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨k + 8, by
            change k + 8 < k + 10 + frag_f.numRegs
            have : 0 < frag_f.numRegs := frag_f.numRegs_pos
            omega⟩ : Fin P.numRegs) = r := Fin.ext rfl
      rw [h_idx_eq, h_pres]; exact h_i1.factor_zero
    · -- mulTmp_zero: register `k + 9 < k + 10`.
      let r : Fin P.numRegs := ⟨k + 9, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change k + 9 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨k + 9, by
            change k + 9 < k + 10 + frag_f.numRegs
            have : 0 < frag_f.numRegs := frag_f.numRegs_pos
            omega⟩ : Fin P.numRegs) = r := Fin.ext rfl
      rw [h_idx_eq, h_pres]; exact h_i1.mulTmp_zero
    · -- acc_eq: register `1 < k + 10`.
      let r : Fin P.numRegs := ⟨1, by
        rw [h_numRegs_eq]
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos; omega⟩
      have h_out : r.val < fBase ∨ fBase + frag_f.numRegs ≤ r.val := by
        left; change 1 < k + 10; omega
      have h_pres := h_outside_preserved r h_out
      have h_idx_eq :
          (⟨1, by
            change 1 < k + 10 + frag_f.numRegs
  ...
  · -- Per-step strict PC bound on the outer side.
    ...
    change pcBase + (URMState.runFor frag_f.toURMProgram f_init k').pc
        < bprod_bodyPCBase frag_f + (frag_f.instrs.size - 1)
    change pcBase + (URMState.runFor frag_f.toURMProgram f_init k').pc
        < pcBase + (frag_f.instrs.size - 1)
    omega

set_option maxHeartbeats 1000000 in
-- Mirrors `compileFrag_bsum_accUpdateBlock_instr_at`: a single
-- segment-peeling helper that exposes the two epilogue instructions
-- (`incR vI` at `bprod_incIPC` and `goto topPC` at `bprod_gotoTopPC`)
-- used by `compileFrag_bprod_partial_phase_i3`. The accUpdate block's
-- 21 internal instruction lookups are already supplied by the existing
-- bundles (`_prep_instr_at`, `_innerBody_instr_at`, `_reset_instr_at`),
-- and `compileFrag_bprod_accUpdate_correct` consumes them internally;
-- this helper provides only the two epilogue lookups that lie outside
-- the accUpdate proper. The heartbeat bump covers reducing the long
-- `prelude ++ loopTop ++ zeroSweep ++ prologue ++ fBody ++ accUpdate
-- ++ epilogue` decomposition through the bounded-array machinery.
/-- Instruction-presence bundle for the two-instruction epilogue of
`compileFrag_bprod`'s per-iteration body: the `incR vI` at
`bprod_incIPC frag_f` and the `goto topPC` (= `jumpZR 0 14 14`) at
`bprod_gotoTopPC frag_f`. Used by
`compileFrag_bprod_partial_phase_i3` to discharge the two
single-step reductions following the accumulator update. -/
private theorem compileFrag_bprod_accUpdateBlock_instr_at
    {k : ℕ}
    (frag_f : CompiledFragment (k + 1)) :
    let outer := (compileFrag_bprod frag_f).toURMProgram
    ∃ (h_vI : k + 4 < outer.numRegs)
      (h_z : (0 : ℕ) < outer.numRegs),
      outer.instrs[bprod_incIPC frag_f]?
          = some (URMInstr.inc ⟨k + 4, h_vI⟩)
        ∧ outer.instrs[bprod_gotoTopPC frag_f]?
            = some (URMInstr.jumpZ ⟨0, h_z⟩ 14 14) := by
  intro outer
  -- Abbreviations matching the constructor of `compileFrag_bprod`.
  let vAcc : ℕ := 1
  let vBoundIn : ℕ := 2
  ...
  have h_preAc_len : preAc.length = trBase := by
    ...
    rw [h_prelude_len, h_loopTop_len, h_zeroSweep_len, h_prologue_len,
      h_fBody_len]
  -- Per-position lookup helper: rawList at PC trBase + d (for d < 23)
  -- equals the corresponding entry of accUpdate ++ epilogue at offset d.
  have h_lookup_raw : ∀ (d : ℕ) (_hd : d < 23) (h_idx_lt : trBase + d
      < rawList.length),
      rawList[trBase + d]'h_idx_lt
        = (accUpdate ++ epilogue)[d]'(by
            rw [List.length_append, h_accUpdate_len, h_epilogue_len]
    ...
    have h_idx_eq : trBase + d - preAc.length = d := by
      rw [h_preAc_len]; omega
    congr 1
  have h_raw_len_eq : rawList.length = preAc.length + 21 + 3 := by
    change (prelude ++ loopTop ++ zeroSweep ++ prologue ++ fBody
      ++ accUpdate ++ epilogue).length = preAc.length + 21 + 3
    rw [h_preAc_def]
    simp only [List.length_append]
    rw [h_accUpdate_len, h_epilogue_len]
  have h_idx_lt_outer_21 : trBase + 21 < rawList.length := by
    rw [h_raw_len_eq, h_preAc_len]; omega
  have h_idx_lt_outer_22 : trBase + 22 < rawList.length := by
    rw [h_raw_len_eq, h_preAc_len]; omega
  refine ⟨?_, ?_⟩
  · -- h_inc_at: outer.instrs[bprod_incIPC frag_f]? = some (incR vI).
    have h_ae_21 : (accUpdate ++ epilogue)[(21 : ℕ)]'(by
        rw [List.length_append, h_accUpdate_len, h_epilogue_len]; decide)
        = URMInstrRaw.incR vI := rfl
    change outer.instrs[bprod_incIPC frag_f]? = _
    rw [show bprod_incIPC frag_f = trBase + 21 from rfl]
    change (URMInstrRaw.toBoundedArray nR rawList hBoundOuter)[
        trBase + 21]? = _
    rw [URMInstrRaw.toBoundedArray_getElem? nR rawList hBoundOuter
        (trBase + 21) h_idx_lt_outer_21]
    have h_raw_eq := h_lookup_raw 21 (by decide) h_idx_lt_outer_21
    have h_combined : rawList[trBase + 21]'h_idx_lt_outer_21
        = URMInstrRaw.incR vI := by
      rw [h_raw_eq]; exact h_ae_21
    apply congrArg some
    exact URMInstrRaw.toBounded_congr nR h_combined _ _
  · -- h_goto_at: outer.instrs[bprod_gotoTopPC frag_f]? = some (jumpZ 0 14 14).
    have h_ae_22 : (accUpdate ++ epilogue)[(22 : ℕ)]'(by
        rw [List.length_append, h_accUpdate_len, h_epilogue_len]; decide)
        = URMInstrRaw.jumpZR 0 14 14 := rfl
    change outer.instrs[bprod_gotoTopPC frag_f]? = _
    rw [show bprod_gotoTopPC frag_f = trBase + 22 from rfl]
    change (URMInstrRaw.toBoundedArray nR rawList hBoundOuter)[
        trBase + 22]? = _
    rw [URMInstrRaw.toBoundedArray_getElem? nR rawList hBoundOuter
        (trBase + 22) h_idx_lt_outer_22]
    have h_raw_eq := h_lookup_raw 22 (by decide) h_idx_lt_outer_22
    have h_combined : rawList[trBase + 22]'h_idx_lt_outer_22
        = URMInstrRaw.jumpZR 0 14 14 := by
      rw [h_raw_eq]; exact h_ae_22
    apply congrArg some
    exact URMInstrRaw.toBounded_congr nR h_combined _ _

set_option maxHeartbeats 4000000 in
-- The 11-clause discharge of `compileFrag_bprod_partial_invariant @
-- (i.val + 1)` plus the three-phase composition (accUpdate + incR vI
-- + goto topPC) drives a substantial whnf load on the bprod numRegs
-- profile; the elaborator's default budget is exhausted.
/-- Phase i.3 preservation lemma for `compileFrag_bprod`: from
`compileFrag_bprod_phase_i2_post @ i`, running for
`T0 := ((4 * A_i + 1) + (4 * B_i + 1) + B_i * (9 * A_i + 5) + 1 + 1)
+ 2` further steps (the accumulator-update `R^XY_Z` block, the
`incR vI`, and the `goto topPC`) lands the state in
`compileFrag_bprod_partial_invariant @ (i.val + 1)`, where
`A_i = natBProd i.val (fun j => f.interp (Fin.cons j.val (Fin.tail v)))`
is the pre-iteration accumulator and `B_i = f.interp (Fin.cons i.val
(Fin.tail v))` is the iteration's f-value. The accompanying strict PC
bound states that during these `T0` steps the intermediate PC stays
strictly less than `(compileFrag_bprod frag_f).instrs.size - 1`
(= `bprod_exitPC frag_f`). The proof composes
`compileFrag_bprod_accUpdate_correct` over the accUpdate's full step
count with single `step` reductions for the `incR vI` and
`goto topPC` instructions, dispatched via
`compileFrag_bprod_accUpdateBlock_instr_at`. The multiplicative
accumulator recurrence `natBProd (i.val + 1) g = natBProd i.val g *
g i.val` discharges `acc_eq` at `i.val + 1`. -/
private theorem compileFrag_bprod_partial_phase_i3
    ...
    (∀ k' < T0,
      (URMState.runFor _ sPre k').pc
        < (compileFrag_bprod (compileERFrag f)).instrs.size - 1) := by
  intro A_i B_i T0
  -- Abbreviations matching the outer program.
  let frag_f : CompiledFragment (k + 1) := compileERFrag f
  let outerFrag : CompiledFragment (k + 1) := compileFrag_bprod frag_f
  let P : URMProgram (k + 1) := outerFrag.toURMProgram
  let T_acc : ℕ :=
    (4 * A_i + 1) + (4 * B_i + 1) + B_i * (9 * A_i + 5) + 1 + 1
  change compileFrag_bprod_partial_invariant frag_f f v (i.val + 1)
      (Nat.succ_le_of_lt i.isLt)
      (URMState.runFor P sPre (T_acc + 2)) ∧
    (∀ k' < T_acc + 2,
      (URMState.runFor P sPre k').pc
        < (compileFrag_bprod frag_f).instrs.size - 1)
  have h_numRegs_eq : P.numRegs = (k + 10) + frag_f.numRegs := rfl
  have h_numRegs_pos : 0 < P.numRegs := outerFrag.numRegs_pos
  -- Instruction-presence bundle for the epilogue.
  obtain ⟨h_vI_lt, h_z_lt, h_inc_at, h_goto_at⟩ :=
    compileFrag_bprod_accUpdateBlock_instr_at frag_f
  -- Pre-state hypotheses from h_i2 with renamed witnesses.
  have h_s_pc : sPre.pc = bprod_trBase frag_f := h_i2.pc_eq
  have h_s_z : sPre.regs ⟨0,
      (compileFrag_bprod frag_f).numRegs_pos⟩ = 0 := h_i2.zReg_zero
  have h_s_acc : sPre.regs ⟨1, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change 1 < k + 10 + frag_f.numRegs; omega⟩ = A_i := h_i2.acc_eq
  have h_s_accClone : sPre.regs ⟨k + 7, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change k + 7 < k + 10 + frag_f.numRegs; omega⟩ = 0 :=
    h_i2.accClone_zero
  have h_s_fOut : sPre.regs ⟨(k + 10) + frag_f.outputReg.val, by
      have hO : frag_f.outputReg.val < frag_f.numRegs :=
        frag_f.outputReg.isLt
      change (k + 10) + frag_f.outputReg.val
        < k + 10 + frag_f.numRegs
      omega⟩ = B_i := h_i2.f_output
  have h_s_factor : sPre.regs ⟨k + 8, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change k + 8 < k + 10 + frag_f.numRegs; omega⟩ = 0 :=
    h_i2.factor_zero
  have h_s_mulTmp : sPre.regs ⟨k + 9, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change k + 9 < k + 10 + frag_f.numRegs; omega⟩ = 0 :=
    h_i2.mulTmp_zero
  -- Step A: accUpdate over T_acc steps.
  obtain ⟨hA_pc, hA_acc, hA_accClone, hA_factor, hA_fOut,
      hA_mulTmp, hA_other⟩ :=
    compileFrag_bprod_accUpdate_correct frag_f A_i B_i sPre h_s_pc h_s_z
      h_s_acc h_s_accClone h_s_fOut h_s_factor h_s_mulTmp
  set sA : URMState P := URMState.runFor P sPre T_acc with h_sA_def
  -- sA register summaries.
  have hsA_pc : sA.pc = bprod_incIPC frag_f := hA_pc
  have hsA_acc : sA.regs ⟨1, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change 1 < k + 10 + frag_f.numRegs; omega⟩ = A_i * B_i := hA_acc
  have hsA_accClone : sA.regs ⟨k + 7, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change k + 7 < k + 10 + frag_f.numRegs; omega⟩ = 0 := hA_accClone
  have hsA_factor : sA.regs ⟨k + 8, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change k + 8 < k + 10 + frag_f.numRegs; omega⟩ = 0 := hA_factor
  have hsA_fOut : sA.regs ⟨(k + 10) + frag_f.outputReg.val, by
      have hO : frag_f.outputReg.val < frag_f.numRegs :=
        frag_f.outputReg.isLt
      change (k + 10) + frag_f.outputReg.val
        < k + 10 + frag_f.numRegs
      omega⟩ = 0 := hA_fOut
  have hsA_mulTmp : sA.regs ⟨k + 9, by
      have : 0 < frag_f.numRegs := frag_f.numRegs_pos
      change k + 9 < k + 10 + frag_f.numRegs; omega⟩ = 0 := hA_mulTmp
  have hsA_other : ∀ r : Fin P.numRegs,
      r.val ≠ 1 →
      r.val ≠ k + 7 →
      r.val ≠ k + 8 →
      r.val ≠ k + 9 →
      r.val ≠ (k + 10) + frag_f.outputReg.val →
      sA.regs r = sPre.regs r := hA_other
  -- Fin handles for the epilogue.
  let vIFin : Fin P.numRegs := ⟨k + 4, h_vI_lt⟩
  let zFin : Fin P.numRegs := ⟨0, h_z_lt⟩
  -- Step B: incR vI. PC = bprod_incIPC frag_f.
  have h_inc_at' :
      P.instrs[sA.pc]? = some (URMInstr.inc vIFin) := by
    rw [hsA_pc]; exact h_inc_at
  have hB_step :
      URMState.step P sA =
        { pc := sA.pc + 1
  ...
  have hsB_pc : sB.pc = bprod_gotoTopPC frag_f := by
    change sA.pc + 1 = bprod_gotoTopPC frag_f
    rw [hsA_pc]
    change bprod_trBase frag_f + 21 + 1 = bprod_trBase frag_f + 22
    omega
  ...
  have hsB_z : sB.regs zFin = 0 := by
    ...
    rw [hsB_other_of_ne_vI zFin h_ne]
    -- sA.regs zFin = sPre.regs zFin = 0 (zFin not in accUpdate exclusion).
    have h_ne_1 : (zFin : Fin P.numRegs).val ≠ 1 := by
      change (0 : ℕ) ≠ 1; decide
    have h_ne_kp7 : (zFin : Fin P.numRegs).val ≠ k + 7 := by
      change (0 : ℕ) ≠ k + 7; omega
    have h_ne_kp8 : (zFin : Fin P.numRegs).val ≠ k + 8 := by
      change (0 : ℕ) ≠ k + 8; omega
    have h_ne_kp9 : (zFin : Fin P.numRegs).val ≠ k + 9 := by
      change (0 : ℕ) ≠ k + 9; omega
    have h_ne_fOut : (zFin : Fin P.numRegs).val ≠
        (k + 10) + frag_f.outputReg.val := by
      change (0 : ℕ) ≠ (k + 10) + frag_f.outputReg.val; omega
    have h_pres := hsA_other zFin h_ne_1 h_ne_kp7 h_ne_kp8 h_ne_kp9
      h_ne_fOut
    rw [h_pres]
    have h_idx_eq :
        (⟨0, (compileFrag_bprod frag_f).numRegs_pos⟩ : Fin P.numRegs)
          = zFin := Fin.ext rfl
    rw [← h_idx_eq]; exact h_s_z
  -- Step C: goto topPC (jumpZR 0 14 14).
  have h_goto_at' :
      P.instrs[sB.pc]? = some (URMInstr.jumpZ zFin 14 14) := by
    rw [hsB_pc]; exact h_goto_at
  have hC_step :
      URMState.step P sB =
        { pc := if sB.regs zFin = 0 then 14 else 14
          regs := sB.regs } :=
    URMState.step_of_getElem?_jumpZ P sB sB.pc zFin 14 14 rfl h_goto_at'
  set sC : URMState P :=
    { pc := if sB.regs zFin = 0 then 14 else 14
      regs := sB.regs } with h_sC_def
  ...
  have hsC_pc : sC.pc = 14 := by
    change (if sB.regs zFin = 0 then 14 else 14) = 14
    split <;> rfl
  -- runFor P sPre (T_acc + 2) = sC via runFor_add.
  have h_runFor_total : URMState.runFor P sPre (T_acc + 2) = sC := by
    have h_eq : T_acc + 2 = T_acc + 1 + 1 := by omega
  ...
  refine ⟨?_, ?_⟩
  · -- compileFrag_bprod_partial_invariant @ (i.val + 1).
    rw [h_runFor_total]
    -- Distinctness facts for vIFin vs accUpdate-excluded registers.
    have h_ne_vI_1 : (vIFin : Fin P.numRegs).val ≠ 1 := by
      change (k + 4 : ℕ) ≠ 1; omega
    have h_ne_vI_kp7 : (vIFin : Fin P.numRegs).val ≠ k + 7 := by
      change (k + 4 : ℕ) ≠ k + 7; omega
    have h_ne_vI_kp8 : (vIFin : Fin P.numRegs).val ≠ k + 8 := by
      change (k + 4 : ℕ) ≠ k + 8; omega
    have h_ne_vI_kp9 : (vIFin : Fin P.numRegs).val ≠ k + 9 := by
      change (k + 4 : ℕ) ≠ k + 9; omega
    have h_ne_vI_fOut : (vIFin : Fin P.numRegs).val ≠
        (k + 10) + frag_f.outputReg.val := by
      change (k + 4 : ℕ) ≠ (k + 10) + frag_f.outputReg.val; omega
    -- sA.regs vIFin = sPre.regs vIFin (vIFin not in accUpdate exclusion).
    have hsA_vI : sA.regs vIFin = sPre.regs vIFin :=
      hsA_other vIFin h_ne_vI_1 h_ne_vI_kp7 h_ne_vI_kp8 h_ne_vI_kp9
        h_ne_vI_fOut
    ...
    refine
      { hi_le := Nat.succ_le_of_lt i.isLt
        ...
        mulTmp_zero := ?_
        acc_eq := ?_ }
    · -- pc_eq: sC.pc = bprod_topPC = 14.
      change sC.pc = bprod_topPC
      change sC.pc = 14
      exact hsC_pc
    · -- zReg_zero: sC.regs ⟨0, _⟩ = 0.
      ...
      exact hsB_z
    · -- outer_inputs: each input slot ⟨2 + j.val, _⟩ preserved.
      intro j
      have hj : j.val < k + 1 := j.isLt
      let r : Fin P.numRegs := ⟨2 + j.val, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_1 : r.val ≠ 1 := by
        change (2 + j.val : ℕ) ≠ 1; omega
      have h_ne_kp7 : r.val ≠ k + 7 := by
        change (2 + j.val : ℕ) ≠ k + 7; omega
      have h_ne_kp8 : r.val ≠ k + 8 := by
        change (2 + j.val : ℕ) ≠ k + 8; omega
      have h_ne_kp9 : r.val ≠ k + 9 := by
        change (2 + j.val : ℕ) ≠ k + 9; omega
      have h_ne_fOut : r.val ≠ (k + 10) + frag_f.outputReg.val := by
        change (2 + j.val : ℕ) ≠ (k + 10) + frag_f.outputReg.val; omega
      have h_ne_vI : r ≠ vIFin := by
        intro hh
        have : (2 + j.val : ℕ) = k + 4 := congrArg Fin.val hh; omega
      have hsA_r := hsA_other r h_ne_1 h_ne_kp7 h_ne_kp8 h_ne_kp9
        h_ne_fOut
    ...
    · -- vX_eq: sC.regs ⟨k + 3, _⟩ = v 0 - (i.val + 1).
      let r : Fin P.numRegs := ⟨k + 3, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_1 : r.val ≠ 1 := by change (k + 3 : ℕ) ≠ 1; omega
      have h_ne_kp7 : r.val ≠ k + 7 := by
        change (k + 3 : ℕ) ≠ k + 7; omega
      have h_ne_kp8 : r.val ≠ k + 8 := by
        change (k + 3 : ℕ) ≠ k + 8; omega
      have h_ne_kp9 : r.val ≠ k + 9 := by
        change (k + 3 : ℕ) ≠ k + 9; omega
      have h_ne_fOut : r.val ≠ (k + 10) + frag_f.outputReg.val := by
        change (k + 3 : ℕ) ≠ (k + 10) + frag_f.outputReg.val; omega
      have h_ne_vI : r ≠ vIFin := by
        intro hh
    ...
    · -- tmp1_zero: sC.regs ⟨k + 5, _⟩ = 0.
      let r : Fin P.numRegs := ⟨k + 5, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_1 : r.val ≠ 1 := by change (k + 5 : ℕ) ≠ 1; omega
      have h_ne_kp7 : r.val ≠ k + 7 := by
        change (k + 5 : ℕ) ≠ k + 7; omega
      have h_ne_kp8 : r.val ≠ k + 8 := by
        change (k + 5 : ℕ) ≠ k + 8; omega
      have h_ne_kp9 : r.val ≠ k + 9 := by
        change (k + 5 : ℕ) ≠ k + 9; omega
      have h_ne_fOut : r.val ≠ (k + 10) + frag_f.outputReg.val := by
        change (k + 5 : ℕ) ≠ (k + 10) + frag_f.outputReg.val; omega
      have h_ne_vI : r ≠ vIFin := by
        intro hh
    ...
    · -- tmp2_zero: sC.regs ⟨k + 6, _⟩ = 0.
      let r : Fin P.numRegs := ⟨k + 6, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_1 : r.val ≠ 1 := by change (k + 6 : ℕ) ≠ 1; omega
      have h_ne_kp7 : r.val ≠ k + 7 := by
        change (k + 6 : ℕ) ≠ k + 7; omega
      have h_ne_kp8 : r.val ≠ k + 8 := by
        change (k + 6 : ℕ) ≠ k + 8; omega
      have h_ne_kp9 : r.val ≠ k + 9 := by
        change (k + 6 : ℕ) ≠ k + 9; omega
      have h_ne_fOut : r.val ≠ (k + 10) + frag_f.outputReg.val := by
        change (k + 6 : ℕ) ≠ (k + 10) + frag_f.outputReg.val; omega
      have h_ne_vI : r ≠ vIFin := by
        intro hh
    ...
    · -- accClone_zero: sC.regs ⟨k + 7, _⟩ = 0 from accUpdate output.
      let r : Fin P.numRegs := ⟨k + 7, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_vI : r ≠ vIFin := by
        intro hh
        have : (k + 7 : ℕ) = k + 4 := congrArg Fin.val hh; omega
      have hsB_r := hsB_other_of_ne_vI r h_ne_vI
      have h_idx_eq :
          (⟨k + 7, by
            change k + 7 < k + 10 + frag_f.numRegs
      ...
      rw [hsB_r]
      exact hsA_accClone
    · -- factor_zero: sC.regs ⟨k + 8, _⟩ = 0 from accUpdate output.
      let r : Fin P.numRegs := ⟨k + 8, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_vI : r ≠ vIFin := by
        intro hh
        have : (k + 8 : ℕ) = k + 4 := congrArg Fin.val hh; omega
      have hsB_r := hsB_other_of_ne_vI r h_ne_vI
      have h_idx_eq :
          (⟨k + 8, by
            change k + 8 < k + 10 + frag_f.numRegs
      ...
      rw [hsB_r]
      exact hsA_factor
    · -- mulTmp_zero: sC.regs ⟨k + 9, _⟩ = 0 from accUpdate output.
      let r : Fin P.numRegs := ⟨k + 9, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_vI : r ≠ vIFin := by
        intro hh
        have : (k + 9 : ℕ) = k + 4 := congrArg Fin.val hh; omega
      have hsB_r := hsB_other_of_ne_vI r h_ne_vI
      have h_idx_eq :
          (⟨k + 9, by
            change k + 9 < k + 10 + frag_f.numRegs
      ...
      rw [hsB_r]
      exact hsA_mulTmp
    · -- acc_eq: sC.regs ⟨1, _⟩ = natBProd (i.val + 1) g.
      let r : Fin P.numRegs := ⟨1, by
        have : 0 < frag_f.numRegs := frag_f.numRegs_pos
        rw [h_numRegs_eq]; omega⟩
      have h_ne_vI : r ≠ vIFin := by
        intro hh
        have : (1 : ℕ) = k + 4 := congrArg Fin.val hh; omega
      have hsB_r := hsB_other_of_ne_vI r h_ne_vI
      have h_idx_eq :
          (⟨1, by
            change 1 < k + 10 + frag_f.numRegs
  ...
  · -- Per-step strict PC bound on the combined interval.
    intro k' hk'
    -- size - 1 = bprod_exitPC frag_f = bprod_trBase frag_f + 23.
    have h_size_eq : (compileFrag_bprod frag_f).instrs.size - 1
        = bprod_exitPC frag_f :=
      (bprod_exitPC_eq_size_pred frag_f).symm
    have h_exit_eq : bprod_exitPC frag_f
        = bprod_trBase frag_f + 23 := rfl
    have h_inc_lt_exit : bprod_incIPC frag_f
        < bprod_trBase frag_f + 23 := by
      change bprod_trBase frag_f + 21 < bprod_trBase frag_f + 23; omega
    have h_goto_lt_exit : bprod_gotoTopPC frag_f
        < bprod_trBase frag_f + 23 := by
      change bprod_trBase frag_f + 22 < bprod_trBase frag_f + 23; omega
    rw [h_size_eq, h_exit_eq]
    -- Case-split on k' relative to T_acc and T_acc + 1.
    rcases Nat.lt_or_ge k' T_acc with hk'_lt_A | hk'_ge_A
    · -- 0 ≤ k' < T_acc: inside accUpdate.
      have h_bound :=
        compileFrag_bprod_accUpdate_pc_strict_bound frag_f A_i B_i sPre
          h_s_pc h_s_z h_s_acc h_s_accClone h_s_fOut h_s_factor
          h_s_mulTmp k' hk'_lt_A
      -- h_bound: pc < bprod_incIPC frag_f < bprod_trBase frag_f + 23.
      exact Nat.lt_trans h_bound h_inc_lt_exit
    · rcases Nat.lt_or_ge k' (T_acc + 1) with hk'_lt_B | hk'_ge_B
      · -- k' = T_acc: state is sA, pc = bprod_incIPC frag_f.
        have h_eq : k' = T_acc := by omega
        rw [h_eq]
        change sA.pc < bprod_trBase frag_f + 23
        rw [hsA_pc]
        exact h_inc_lt_exit
      · -- k' = T_acc + 1: state is sB, pc = bprod_gotoTopPC frag_f.
        have h_eq : k' = T_acc + 1 := by omega
        rw [h_eq]
        rw [URMState.runFor_add, h_runFor_A_B]
        change sB.pc < bprod_trBase frag_f + 23
        rw [hsB_pc]
        exact h_goto_lt_exit

set_option maxHeartbeats 4000000 in
-- Heartbeat bump justified by the four-way composition of phase
-- preservation lemmas, each contributing both a state-equation and a
-- strict PC bound discharged on a separate sub-interval.
/-- Per-iteration induction glue for `compileFrag_bprod`: from
`compileFrag_bprod_partial_invariant @ i.val`, there exists a combined
step count `T0` after which `compileFrag_bprod_partial_invariant @
(i.val + 1)` holds, and throughout these `T0` steps the intermediate
PC stays strictly below `(compileFrag_bprod frag_f).instrs.size - 1`
(= `bprod_exitPC frag_f`). The witness `T0` decomposes as `T1 + T2 +
T3 + T4`: phase i.0 (`2 + numRegs_f`), phase i.1
(`9 * vPrefixSum (Fin.cons i.val (Fin.tail v)) (k + 1) + 2 * (k + 1)`),
phase i.2 (existential, `≤ compileER_runtime f (Fin.cons i.val
(Fin.tail v))`), and phase i.3
(`(4 * A_i + 1) + (4 * B_i + 1) + B_i * (9 * A_i + 5) + 1 + 1 + 2`),
with `A_i = natBProd i.val (fun j => f.interp (Fin.cons j (Fin.tail v)))`
and `B_i = f.interp (Fin.cons i.val (Fin.tail v))`. The structural
induction hypothesis on `f` is threaded into phase i.2; the other
...
private theorem compileFrag_bprod_partial_step
  ...
  change ∃ T0 : ℕ,
    ...
    ∧ compileFrag_bprod_partial_invariant frag_f f v
        (i.val + 1) (Nat.succ_le_of_lt i.isLt)
  ...
  obtain ⟨h_i0_post, h_strict_i0⟩ :=
    compileFrag_bprod_partial_phase_i0 f v i sPre h_inv
  ...
  obtain ⟨h_i1_post, h_strict_i1⟩ :=
    compileFrag_bprod_partial_phase_i1 f v i s1 h_i0_post
  ...
  obtain ⟨T3, hT3_le, h_i2_post, h_strict_i2⟩ :=
    compileFrag_bprod_partial_phase_i2 f ih_f v i s2 h_i1_post
  ...
  obtain ⟨h_inv_succ, h_strict_i3⟩ :=
    compileFrag_bprod_partial_phase_i3 f v i s3 h_i2_post
  -- Total step count.
  ...
  · -- Per-step strict PC bound on the combined interval.
    intro k' hk'
    -- PC layout: prologueBase ≤ bodyPCBase ≤ trBase ≤ exitPC.
    have h_exitPC_size :
        bprod_exitPC frag_f
          = (compileFrag_bprod frag_f).instrs.size - 1 :=
      bprod_exitPC_eq_size_pred frag_f
    have h_exitPC_trBase :
        (compileFrag_bprod frag_f).instrs.size - 1
          = bprod_trBase frag_f + 23 := by
      rw [← h_exitPC_size]; rfl
    have h_prologueBase_lt_exit :
        bprod_prologueBase frag_f
          < (compileFrag_bprod frag_f).instrs.size - 1 := by
      rw [h_exitPC_trBase]
      change 16 + frag_f.numRegs < bprod_trBase frag_f + 23
      change 16 + frag_f.numRegs
        < (16 + frag_f.numRegs + 9 * (k + 1)
            + (frag_f.instrs.size - 1)) + 23
      omega
    have h_bodyPCBase_lt_exit :
        bprod_bodyPCBase frag_f
          < (compileFrag_bprod frag_f).instrs.size - 1 := by
      rw [h_exitPC_trBase]
      change 16 + frag_f.numRegs + 9 * (k + 1)
        < (16 + frag_f.numRegs + 9 * (k + 1)
            + (frag_f.instrs.size - 1)) + 23
      omega
    have h_trBase_lt_exit :
        bprod_trBase frag_f
          < (compileFrag_bprod frag_f).instrs.size - 1 := by
      rw [h_exitPC_trBase]; omega
    -- Phase i.0 window: k' < T1.
    rcases Nat.lt_or_ge k' T1 with h1 | h1
    · have h_bound : (URMState.runFor outer sPre k').pc
          < bprod_prologueBase frag_f := h_strict_i0 k' h1
      exact Nat.lt_trans h_bound h_prologueBase_lt_exit
    · rcases Nat.lt_or_ge k' (T1 + T2) with h2 | h2
      · -- Phase i.1 window: T1 ≤ k' < T1 + T2.
        let k'' : ℕ := k' - T1
        have hk''_lt : k'' < T2 := by
        ...
        have h_bound : (URMState.runFor outer s1 k'').pc
            < bprod_bodyPCBase frag_f := h_strict_i1 k'' hk''_lt
        rw [h_runFor_split]
        exact Nat.lt_trans h_bound h_bodyPCBase_lt_exit
      · rcases Nat.lt_or_ge k' (T1 + T2 + T3) with h3 | h3
        · -- Phase i.2 window: T1 + T2 ≤ k' < T1 + T2 + T3.
          let k'' : ℕ := k' - T1 - T2
          have hk''_lt : k'' < T3 := by
          ...
          have h_bound : (URMState.runFor outer s2 k'').pc
              < bprod_trBase frag_f := h_strict_i2 k'' hk''_lt
          rw [h_runFor_split]
          exact Nat.lt_trans h_bound h_trBase_lt_exit
        · -- Phase i.3 window: T1 + T2 + T3 ≤ k' < T1 + T2 + T3 + T4.
          let k'' : ℕ := k' - T1 - T2 - T3
          have hk''_lt : k'' < T4 := by
...
set_option maxHeartbeats 1000000 in
-- The prelude step ladder threads the four `assignR`s, the
-- `preservingTransfer` block, and the final `incR vAcc` through a
-- cumulative whnf load that exhausts the default heartbeats budget.
/-- Per-step strict PC bound for the prelude of `compileFrag_bprod`:
during the `7 + 9 * (v 0)` step ladder that runs the four
`assignR` instructions, the `9 * (v 0) + 2`-step
`preservingTransfer` block, and the trailing `incR vAcc`, the PC
stays strictly below `(compileFrag_bprod (compileERFrag f)).instrs.size - 1`.
Mirrors `compileFrag_bsum_prelude_pc_strict_bound`, with the
extra step accounting for bprod's `incR vAcc` (7 + 9 v 0 total
vs bsum's 6 + 9 v 0). -/
private theorem compileFrag_bprod_prelude_pc_strict_bound
  ...
  change (URMState.runFor P s0 k').pc < (compileFrag_bprod frag_f).instrs.size - 1
  ...
  have h_exitPC_expand :
      (compileFrag_bprod frag_f).instrs.size - 1
        = 16 + frag_f.numRegs + 9 * (k + 1)
            + (frag_f.instrs.size - 1) + 23 := by
    rw [← h_exitPC_size]
    change bprod_trBase frag_f + 23 = _
    change bprod_bodyPCBase frag_f + (frag_f.instrs.size - 1) + 23 = _
    rfl
  -- Outer numRegs equals `(k + 10) + frag_f.numRegs`.
  have h_numRegs_eq : P.numRegs = (k + 10) + frag_f.numRegs := rfl
  have h_numRegs_pos : 0 < P.numRegs := outerFrag.numRegs_pos
  ...
  -- Prelude instruction lookups at PCs 0..3 (each `assignR`) and PC 13 (the `incR vAcc`).
  have h_outer_at_0 : P.instrs[(0 : ℕ)]? = some (URMInstr.assign rZ 0) := rfl
  have h_outer_at_1 : P.instrs[(1 : ℕ)]? = some (URMInstr.assign rAcc 0) := rfl
  have h_outer_at_2 : P.instrs[(2 : ℕ)]? = some (URMInstr.assign rVX 0) := rfl
  have h_outer_at_3 : P.instrs[(3 : ℕ)]? = some (URMInstr.assign rVI 0) := rfl
  ...
  have h_outer_inputRegs : ∀ (j : Fin (k + 1)),
      (P.inputRegs j).val = 2 + j.val := fun _ => rfl
  -- Step ladder s0 → s1 → s2 → s3 → s4 (the four `assignR`s).
  have hs0_pc : s0.pc = 0 := rfl
  have h_step0 :
      URMState.step P s0 =
        { pc := 1
  ...
  have h_runFor_s2_1 : URMState.runFor P s2 1 = s3 := by
    change URMState.runFor P (URMState.step P s2) 0 = _
    rw [URMState.runFor_zero, h_step2]
  have h_runFor_s3_1 : URMState.runFor P s3 1 = s4 := by
    change URMState.runFor P (URMState.step P s3) 0 = _
    rw [URMState.runFor_zero, h_step3]
  have h_runFor_4 : URMState.runFor P s0 4 = s4 := by
    have h_eq : (4 : ℕ) = 1 + 1 + 1 + 1 := by omega
    rw [h_eq]
    rw [URMState.runFor_add, URMState.runFor_add, URMState.runFor_add]
    rw [h_runFor_1, h_runFor_s1_1, h_runFor_s2_1, h_runFor_s3_1]
  have hs4_pc : s4.pc = 4 := rfl
  ...
  have hs4_rBoundIn : s4.regs rBoundIn = v 0 := by
    ...
    exact hs0_rBoundIn
  -- Case split into prelude windows.
  ...
  · -- k' ∈ {0, 1, 2, 3}: PC at step k' is k', strictly below the exit PC.
    ...
    omega
  · -- 4 ≤ k' < 7 + 9 * (v 0): split into preservingTransfer window and final incR.
    rcases Nat.lt_or_ge k' (4 + (9 * (v 0) + 2)) with h_pT_lt | h_pT_ge
    · -- preservingTransfer window: 4 ≤ k' < 4 + 9 * (v 0) + 2.
      let k'' : ℕ := k' - 4
      ...
      set sz : ℕ := frag_f.instrs.size - 1
      omega
    · -- 4 + 9 * (v 0) + 2 ≤ k' < 7 + 9 * (v 0): single incR step at PC 13.
      -- k' must equal 6 + 9 * (v 0).
      have h_k'_eq : k' = 6 + 9 * (v 0) := by
        change 4 + (9 * (v 0) + 2) ≤ k' at h_pT_ge; omega
      -- s5 := runFor s4 (9 * (v 0) + 2) is the state after preservingTransfer.
      obtain ⟨pt_pc, _, _, _, _, _⟩ :=
        preservingTransfer_correct P 4 rBoundIn rVX rTmp2 rZ
          h_disj_sd h_disj_st h_disj_dt h_disj_zs h_disj_zd h_disj_zt
          H_pT s4 hs4_pc hs4_rZ hs4_rTmp2 (v 0) hs4_rBoundIn
      let s5 : URMState P := URMState.runFor P s4 (9 * (v 0) + 2)
      have hs5_pc : s5.pc = 13 := by
        have h_eq : (4 + 9 : ℕ) = 13 := by omega
        change (URMState.runFor P s4 (9 * (v 0) + 2)).pc = 13
        rw [← h_eq]; exact pt_pc
      -- Rewrite `URMState.runFor P s0 k'` as `s5` directly.
      have h_runFor_k'_eq : URMState.runFor P s0 k' = s5 := by
        rw [h_k'_eq]
        have h_arith : (6 + 9 * (v 0) : ℕ) = 4 + (9 * (v 0) + 2) := by omega
        rw [h_arith, URMState.runFor_add, h_runFor_4]
      rw [h_runFor_k'_eq]
      change s5.pc < _
      rw [hs5_pc, h_exitPC_expand]
      have h_pos_numRegs : 0 < frag_f.numRegs := frag_f.numRegs_pos
      set nR : ℕ := frag_f.numRegs
      set sz : ℕ := frag_f.instrs.size - 1
      omega
...
set_option maxHeartbeats 4000000 in
-- The aux lemma carries an `i`-induction with nested case splits on
-- `k' < T_i'` vs `k' ≥ T_i'`, and a List.range_succ unfolding for the
-- bound. The heartbeats budget exceeds the default.
/-- Strengthened outer iteration of `compileFrag_bprod`'s pre-stop
trace: for every `i ≤ v 0`, there exists a combined step count `T0`
after which `compileFrag_bprod_partial_invariant @ i` holds, and
throughout these `T0` steps the intermediate PC stays strictly
below `(compileFrag_bprod (compileERFrag f)).instrs.size - 1`
(= `bprod_exitPC frag_f`). The bound `T0 ≤ (7 + 9 * (v 0))
+ Σ_{j < i} perIter j` decomposes as the prelude cost (from
`compileFrag_bprod_partial_base`) plus a `List.range i`-indexed sum
of per-iteration costs (each iteration's cost from
`compileFrag_bprod_partial_step`). Specialising `i := v 0`
recovers the unbounded form `compileFrag_bprod_partial`. -/
private theorem compileFrag_bprod_partial_aux
  ...
  induction i with
  | zero =>
    refine ⟨7 + 9 * (v 0), ?_, ?_, ?_⟩
    · -- Bound: List.range 0 = []; foldl on empty is 0.
      change 7 + 9 * (v 0) ≤ (7 + 9 * (v 0))
        + (([] : List ℕ).map perIter).foldl (· + ·) 0
      simp
    · -- partial_invariant @ 0 from the base case.
      exact compileFrag_bprod_partial_base f v
    · -- Strict PC bound across the prelude.
      intro k' hk'
      exact compileFrag_bprod_prelude_pc_strict_bound f v k' hk'
  | succ i' ih =>
    ...
    obtain ⟨T_step, hT_step_bound, h_inv_succ, h_strict_step⟩ :=
      compileFrag_bprod_partial_step f ih_f v i_fin sPre h_inv_i'
    refine ⟨T_i' + T_step, ?_, ?_, ?_⟩
    · -- Bound: T_i' + T_step ≤ (7 + 9 * (v 0))
      ...
      have h_foldl_snoc : ∀ (l : List ℕ) (x : ℕ),
        ...
        rfl
      rw [h_foldl_snoc]
      -- T_step ≤ perIter i' (definitionally identical at i' = i_fin.val).
      have hT_step_bound' :
          T_step ≤ (2 + frag_f.numRegs)
            + (9 * vPrefixSum (Fin.cons i_fin.val (Fin.tail v)) (k + 1)
                + 2 * (k + 1))
            + compileER_runtime f (Fin.cons i_fin.val (Fin.tail v))
            + ((4 * natBProd i_fin.val
                  (fun j => f.interp (Fin.cons j (Fin.tail v))) + 1)
                + (4 * f.interp (Fin.cons i_fin.val (Fin.tail v)) + 1)
                + f.interp (Fin.cons i_fin.val (Fin.tail v))
                    * (9 * natBProd i_fin.val
                        (fun j => f.interp (Fin.cons j (Fin.tail v))) + 5)
                + 1 + 1 + 2) :=
        hT_step_bound
      have h_perIter_eq :
...
/-- Outer iteration of `compileFrag_bprod`'s pre-stop trace at
`i = v 0`: thin specialisation of `compileFrag_bprod_partial_aux`
at the unbounded form, producing the
`compileFrag_bprod_partial_invariant @ v 0` state (loop counter
exhausted, accumulator equal to the total `bprod`-product) ready
to be threaded through the trailing `jumpZR vX exitPC` by the
final assembly `compileER_pre_stop_correct_bprod`. -/
...
/-- `≤ t'`-form wrapper around `compileER_pre_stop_correct_bprod`,
analogue of `compileER_runFor_bsum`. Given the pre-stop inductive
hypothesis for `f`, running `compileER (.bprod f)` for any
`t' ≥ compileER_runtime (.bprod f) v` produces `(.bprod f).interp v`
in the output register. -/
private theorem compileER_runFor_bprod {k : ℕ}
    (f : ERMor1 (k + 1))
    (ih_f : ∀ (v' : Fin (k + 1) → ℕ),
      ∃ T0 : ℕ,
        T0 ≤ compileER_runtime f v' ∧
        ...
        (∀ k' < T0,
          (URMState.runFor (compileER f)
            ...
            < (compileER f).instrs.size - 1))
    (v : Fin (k + 1) → ℕ) (t' : ℕ)
    (ht' : compileER_runtime (.bprod f : ERMor1 (k + 1)) v ≤ t') :
    (URMState.runFor (compileER (.bprod f : ERMor1 (k + 1)))
        (URMState.init (compileER (.bprod f : ERMor1 (k + 1))) v) t').regs
        (compileER (.bprod f : ERMor1 (k + 1))).outputReg
      = (.bprod f : ERMor1 (k + 1)).interp v := by
  obtain ⟨T0, hT0, h_pc, h_out, _⟩ :=
    compileER_pre_stop_correct_bprod f ih_f v
