...
/-- The profunctor Barr lift of an
endoprofunctor `G : Type^op ⥤ Type ⥤ Type` at
a relation `R : I₀ → I₁ → Prop`.

A pair `(d₀, d₁)` of diagonal elements is
related iff there exists a witness
`w ∈ diagApp G S` (where `S` is the subtype
of `R`-related pairs) whose covariant and
contravariant projections recover `d₀` and
`d₁` via `DiagCompat`. This is the
existential formulation of the profunctor
relation lifting, dual to the universal
`arrowRel`-based formulation used in
`TypeExpr.fullRelInterp`. -/
def profBarrLiftRel
    (G : Typeᵒᵖ ⥤ Type ⥤ Type)
    {I₀ I₁ : Type} (R : I₀ → I₁ → Prop)
