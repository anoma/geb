...
`count : D ⟶ p.T` may be arbitrary; it encodes a
unary natural number, and for every such number the
result is the input `init`. -/
theorem iterNat_cfpLift_fixed {D : C}
    (f : p.T ⟶ p.T) (init count : D ⟶ p.T)
    (hfix : init ≫ f = init) :
    cfpLift init count ≫ iterNat f = init := by
  -- Factor the left-hand side through cfpMap init
  -- (𝟙 p.T) and invoke elim_naturality to rewrite
  -- iterNat f applied with a shifted base.
  have factor_lift :
      cfpLift init count =
      cfpLift (𝟙 D) count ≫ cfpMap init (𝟙 p.T) := by
    rw [cfpLift_cfpMap, Category.id_comp,
      Category.comp_id]
  -- Phrase iterNat f via p.elim.
  have unfold_iter :
      (iterNat f : cfpProd p.T p.T ⟶ p.T) =
      p.elim (𝟙 p.T) (cfpSnd p.T p.T ≫ f) := rfl
  rw [factor_lift, Category.assoc, unfold_iter]
  -- elim_naturality: cfpMap init (𝟙 p.T) ≫
  -- p.elim (𝟙 p.T) s = p.elim (init ≫ 𝟙 p.T) s.
  rw [elim_naturality, Category.comp_id]
  -- Goal:
  -- cfpLift (𝟙 D) count ≫ p.elim init
  --   (cfpSnd p.T p.T ≫ f) = init.
  -- Prove p.elim init (cfpSnd p.T p.T ≫ f) =
  -- cfpFst D p.T ≫ init via elim_uniq.
  have claim :
      (cfpFst D p.T ≫ init :
        cfpProd D p.T ⟶ p.T) =
      p.elim init (cfpSnd p.T p.T ≫ f) := by
    apply p.elim_uniq init (cfpSnd p.T p.T ≫ f)
    · -- Base: cfpInsertSnd p.ℓ D ≫
      -- (cfpFst D p.T ≫ init) = init.
      unfold cfpInsertSnd
      rw [← Category.assoc, cfpLift_fst,
        Category.id_comp]
    · -- Step: cfpMap (𝟙 D) p.β ≫
      -- (cfpFst D p.T ≫ init) =
      -- cfpLiftAssoc (cfpFst D p.T ≫ init)
      --   (cfpFst D p.T ≫ init) ≫
      --   (cfpSnd p.T p.T ≫ f).
      unfold cfpMap
      rw [← Category.assoc, cfpLift_fst,
        Category.assoc, Category.id_comp]
      -- Goal: cfpFst D (cfpProd p.T p.T) ≫ init =
      --   cfpLiftAssoc (cfpFst D p.T ≫ init)
      --     (cfpFst D p.T ≫ init) ≫
      --     (cfpSnd p.T p.T ≫ f).
      -- Reduce cfpLiftAssoc ≫ cfpSnd p.T p.T.
      rw [show
        cfpLiftAssoc (cfpFst D p.T ≫ init)
            (cfpFst D p.T ≫ init) ≫
          cfpSnd p.T p.T ≫ f =
      ...
      --   cfpFst D p.T ≫ init) ≫ f.
      have h_assoc_fst :
          cfpAssocSnd D p.T p.T ≫ cfpFst D p.T =
  ...
  rw [← claim, ← Category.assoc, cfpLift_fst,
    Category.id_comp]
...
theorem treeEq_ℓβ {D : C} (g1 g2 : D ⟶ p.T) :
    cfpLift (cfpTerminalFrom D ≫ p.ℓ)
        (cfpLift g1 g2 ≫ p.β) ≫ treeEq =
    cfpTerminalFrom D ≫ treeFalse := by
  set x : D ⟶ p.T := cfpTerminalFrom D ≫ p.ℓ
    with hx
  set y : D ⟶ p.T := cfpLift g1 g2 ≫ p.β with hy
  unfold treeEq
  simp only
...
`φ` to the carrier `cfpProd A (cfpProd T X)` by
pairing `(cfpFst, cfpSnd, φ)` and applies `elim_uniq`
on the lifted morphism. -/
theorem paraElim_uniq {A X : C} (f : A ⟶ X)
    (g : cfpProd A
        (cfpProd (cfpProd p.T p.T) (cfpProd X X)) ⟶
      X)
    (φ : cfpProd A p.T ⟶ X)
    ...
    (hβ : cfpMap (𝟙 A) p.β ≫ φ =
      cfpLift
        ...
        g) :
    φ = paraElim f g := by
  -- Lifted φ: (a, t) ↦ (a, (t, φ(a, t))).
  set φLift : cfpProd A p.T ⟶
      paraCarrier (p := p) A X :=
    cfpLift (cfpFst A p.T)
      (cfpLift (cfpSnd A p.T) φ)
    with hφLift
  -- Show φLift = p.elim (paraBase f) (paraStep g).
  have hlift_eq :
      φLift =
      p.elim (paraBase (p := p) f)
        (paraStep (p := p) g) := by
    apply p.elim_uniq
      ...
      (paraStep (p := p) g)
      φLift
    · -- cfpInsertSnd p.ℓ A ≫ φLift = paraBase f.
      rw [hφLift]
      unfold paraBase
      rw [cfpLift_precomp]
      apply cfpLift_uniq
      ...
      · rw [cfpLift_snd, cfpLift_precomp]
        ...
        · rw [cfpLift_fst]
          ...
          rw [cfpLift_snd]
        · rw [cfpLift_snd, hℓ]
    · -- cfpMap (𝟙 A) p.β ≫ φLift =
      -- cfpLiftAssoc φLift φLift ≫ paraStep g.
      rw [hφLift]
      rw [cfpLift_precomp]
      -- LHS is now
      --   cfpLift (cfpMap (𝟙 A) p.β ≫ cfpFst A p.T)
      --     (cfpMap (𝟙 A) p.β ≫ cfpLift (cfpSnd A p.T) φ)
      -- Show the two sides agree by `cfpLift_uniq`
      -- componentwise.
      symm
      apply cfpLift_uniq
      · -- First component: cfpLiftAssoc φLift φLift ≫
        -- paraStep g ≫ cfpFst A (cfpProd T X) =
        -- cfpMap (𝟙 A) p.β ≫ cfpFst A p.T.
        rw [Category.assoc]
        have hstep_fst :
            paraStep (p := p) g ≫
              cfpFst A (cfpProd p.T X) =
        ...
        rw [cfpLift_fst, cfpMap_fst,
          Category.comp_id]
      · -- Second component: cfpLiftAssoc φLift φLift ≫
        -- paraStep g ≫ cfpSnd A (cfpProd T X) = ...
        rw [Category.assoc]
        -- Decompose paraStep ≫ cfpSnd A (cfpProd T X).
        have hstep_snd :
            paraStep (p := p) g ≫
              cfpSnd A (cfpProd p.T X) =
            cfpLift
          ...
          rw [cfpLift_snd]
        rw [hstep_snd]
        -- Distribute both cfpLiftAssoc ≫ big and
        -- cfpMap ≫ cfpLift over their inner cfpLifts.
        rw [cfpLift_precomp, cfpLift_precomp]
        ...
        · -- tree pair side.
          rw [cfpMap_snd]
          -- Helper: φLift ≫ cfpSnd A _ ≫ cfpFst T X =
          -- cfpSnd A p.T.
          have h_φL_fstT :
              cfpLift (cfpFst A p.T)
                  (cfpLift (cfpSnd A p.T) φ) ≫
          ...
          -- then compose with p.β.
          -- cfpLiftAssoc φLift φLift ≫ cfpFst X' X' =
          -- cfpAssocFst A T T ≫ φLift.
          have h_LA_fst :
              cfpLiftAssoc
                  (cfpLift (cfpFst A p.T)
                    (cfpLift (cfpSnd A p.T) φ))
        ...
        · -- g-argument side.
          ...
          rw [← Category.assoc]
          congr 1
          -- Goal: cfpLiftAssoc φLift φLift ≫ gBit =
          -- cfpLift (cfpFst A (T×T))
          --   (cfpLift (cfpSnd A (T×T))
          --     (cfpLiftAssoc φ φ))
          -- Helper: φLift ≫ cfpFst A (cfpProd T X) =
          -- cfpFst A p.T.
          have h_φL_fst :
              cfpLift (cfpFst A p.T)
                  (cfpLift (cfpSnd A p.T) φ) ≫
          ...
          · -- First projection: a component.
            ...
            unfold cfpAssocFst
            rw [cfpLift_fst]
          · -- Second projection: (t×t, x×x) pair.
            rw [Category.assoc, cfpLift_snd]
            apply cfpLift_uniq
            · -- Tree pair.
              rw [Category.assoc, cfpLift_fst,
            ...
            · -- Result pair.
              ...
              · rw [cfpLift_snd, ← Category.assoc,
                  ...
                  h_φL_sndT]
  -- From φLift = p.elim base' step', project to φ.
  have hproj :
      φLift ≫ cfpSnd A (cfpProd p.T X) ≫ cfpSnd p.T X = φ
    := by
    rw [hφLift, ← Category.assoc, cfpLift_snd,
      cfpLift_snd]
  unfold paraElim
  rw [← hlift_eq, hproj]
