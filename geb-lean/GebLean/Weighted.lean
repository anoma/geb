...
section WeightedWedgeDiagElementsEquivalence

/-!
## Profunctor on diagonal elements

Given a weight profunctor `W : Cᵒᵖ ⥤ C ⥤ Type v` and a diagram profunctor
`P : Cᵒᵖ ⥤ C ⥤ D`, we can define a profunctor on the category of diagonal
elements `DiagElem W` by restricting `P` via the forgetful functor.

The resulting profunctor `diagElemProfunctor W P : (DiagElem W)ᵒᵖ ⥤ DiagElem W ⥤ D`
evaluates at objects `x, y : DiagElem W` to `P(x.base, y.base)`.

This construction allows us to potentially view weighted wedges/cowedges
as ordinary wedges/cowedges over the category of diagonal elements.
-/
...
/--
The profunctor on diagonal elements induced by a diagram profunctor.

Given weight `W : Cᵒᵖ ⥤ C ⥤ Type v` and diagram `P : Cᵒᵖ ⥤ C ⥤ D`,
this defines a profunctor on `DiagElem W` by composing `P` with the
forgetful functor `DiagElem.forget W : DiagElem W ⥤ C`.

At objects `x, y : DiagElem W`, we have:
`(diagElemProfunctor W P).obj (op x) |>.obj y = P.obj (op x.base) |>.obj y.base`
-/
def diagElemProfunctor (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D) :
    (DiagElem W)ᵒᵖ ⥤ DiagElem W ⥤ D :=
  (DiagElem.forget W).op ⋙ P ⋙
    (Functor.whiskeringLeft (DiagElem W) C D).obj (DiagElem.forget W)

/-- The object-level computation for `diagElemProfunctor`. -/
@[simp]
theorem diagElemProfunctor_obj_obj (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D)
    (x : (DiagElem W)ᵒᵖ) (y : DiagElem W) :
    ((diagElemProfunctor W P).obj x).obj y =
      (P.obj (Opposite.op x.unop.base)).obj y.base := rfl

/-- The covariant map (second argument) for `diagElemProfunctor`. -/
@[simp]
theorem diagElemProfunctor_obj_map (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D)
    (x : (DiagElem W)ᵒᵖ) {y y' : DiagElem W} (f : y ⟶ y') :
    ((diagElemProfunctor W P).obj x).map f =
      (P.obj (Opposite.op x.unop.base)).map f.base := rfl

/-- The contravariant map (first argument) for `diagElemProfunctor`. -/
@[simp]
theorem diagElemProfunctor_map_app (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D)
    {x x' : (DiagElem W)ᵒᵖ} (f : x ⟶ x') (y : DiagElem W) :
    ((diagElemProfunctor W P).map f).app y =
      (P.map f.unop.base.op).app y.base := rfl

/-!
### Wedges over the diagonal elements profunctor

We define `DiagWedge W P` as wedges over `diagElemProfunctor W P`, and explore
the relationship with `WeightedWedge W P`.
-/

/--
A wedge over the profunctor on diagonal elements. This is an ordinary wedge
for the profunctor `diagElemProfunctor W P : (DiagElem W)ᵒᵖ ⥤ DiagElem W ⥤ D`.

The data consists of:
- An apex `pt : D`
- For each `x : DiagElem W` (i.e., `(c : C, w : W(c,c))`), a morphism
  `pt ⟶ P(c, c)`
- Subject to the wedge (dinaturality) condition.
-/
abbrev DiagWedge (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D) :=
  Wedge (diagElemProfunctor W P)

/--
Extract the leg of a `DiagWedge` at a diagonal element `x`.
-/
def DiagWedge.leg {W : Cᵒᵖ ⥤ C ⥤ Type v₆} {P : Cᵒᵖ ⥤ C ⥤ D}
    (w : DiagWedge W P) (x : DiagElem W) :
    w.pt ⟶ (P.obj (Opposite.op x.base)).obj x.base :=
  w.ι x

/-!
### From weighted wedges to diagonal wedges

Given a weighted wedge, we can extract the "diagonal" components: the legs
at identity twisted arrows. This defines a functor from `WeightedWedge W P`
to `DiagWedge W P`.
-/

/--
Convert a diagonal element of `W` to a twisted arrow object (identity arrow).
-/
def diagElemToTwObj {W : Cᵒᵖ ⥤ C ⥤ Type v₆}
    (x : DiagElem W) : TwistedArrow C :=
  twObjMk (𝟙 x.base)

/--
The weight element at a diagonal element corresponds to the weight at an
identity twisted arrow.
-/
def diagElemToTwWeight {W : Cᵒᵖ ⥤ C ⥤ Type v₆}
    (x : DiagElem W) :
    diagApp W x.base → (profunctorOnTwistedArrow C W).obj (diagElemToTwObj x) := by
  intro w
  simp only [diagElemToTwObj, profunctorOnTwistedArrow_obj, twObjMk_dom, twObjMk_cod]
  exact w

/--
Extract the diagonal components from a weighted wedge.

Given a weighted wedge and a diagonal element `x = (c, w)`, the leg at the
identity twisted arrow `id_c` with weight `w` gives a morphism `pt → P(c, c)`.
-/
def weightedWedgeToDiagWedgeι (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D)
    (c : WeightedWedge W P) (x : DiagElem W) :
    c.pt ⟶ ((diagElemProfunctor W P).obj (Opposite.op x)).obj x :=
  c.leg (diagElemToTwObj x) (diagElemToTwWeight x x.elem)

/--
The diagonal components of a weighted wedge satisfy the wedge condition
for `diagElemProfunctor W P`.

For a morphism `f : x ⟶ y` in `DiagElem W`, the wedge condition is:
`ι x ≫ (P.obj (op x.base)).map f.base = ι y ≫ (P.map f.base.op).app y.base`
-/
theorem weightedWedgeToDiagWedge_condition (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D)
    (c : WeightedWedge W P) {x y : DiagElem W} (f : x ⟶ y) :
    weightedWedgeToDiagWedgeι W P c x ≫
      ((diagElemProfunctor W P).obj (Opposite.op x)).map f =
    weightedWedgeToDiagWedgeι W P c y ≫
      ((diagElemProfunctor W P).map f.op).app y := by
  -- Simplify the profunctor actions
  simp only [diagElemProfunctor_obj_map, diagElemProfunctor_map_app, Quiver.Hom.unop_op]
  -- Goal: ι x ≫ (P.obj (op x.base)).map f.base = ι y ≫ (P.map f.base.op).app y.base
  -- This needs to come from the weighted wedge naturality, but we need to
  -- construct a morphism in TwistedArrow C from id_{x.base} to id_{y.base}
  -- to apply the naturality condition.
  --
  -- A morphism from (id : x.base → x.base) to (id : y.base → y.base) in
  -- TwistedArrow C is given by (domArr := f.base, codArr := f.base) such that
  -- f.base ≫ id ≫ f.base = id. But this requires f.base ≫ f.base = id, which
  -- is not generally true!
  --
  -- This suggests the construction may need refinement.
  sorry

/--
Convert a weighted wedge to a wedge over the diagonal elements profunctor.
-/
def weightedWedgeToDiagWedge (W : Cᵒᵖ ⥤ C ⥤ Type v₆) (P : Cᵒᵖ ⥤ C ⥤ D)
    (c : WeightedWedge W P) : DiagWedge W P :=
  Wedge.mk c.pt (fun x => weightedWedgeToDiagWedgeι W P c x)
    (fun {_x _y} f => weightedWedgeToDiagWedge_condition W P c f)

end WeightedWedgeDiagElementsEquivalence

section WeightedWedgeElementsEquivalence

/-!
## Weighted wedges via the full category of elements

This section explores the relationship between weighted wedges and ordinary
cones using the *full* category of elements of the weight profunctor, rather
than just the diagonal elements.

For a profunctor `W : Cᵒᵖ ⥤ C ⥤ Type v`, the category of elements
`(Functor.uncurry.obj W).Elements` has:
- Objects: pairs `((a, b), w)` where `a : Cᵒᵖ`, `b : C`, and `w : W(a, b)`
- Morphisms: pairs `(f, g)` in `Cᵒᵖ × C` with compatibility on weights

The key observation is that `Tw(C) ≅ El(Hom_C)` where `Hom_C : Cᵒᵖ × C → Set`.
For a general weight `W`, we have a forgetful map from `El(W)` to `Cᵒᵖ × C`,
and restricting `P` along this map gives a functor `El(W) → D`.

A weighted cone/wedge with weight `W` should correspond to an ordinary cone
over this induced diagram.
-/
...
/--
The category of elements of a profunctor, obtained by uncurrying the
bifunctor and taking the standard category of elements.

For `W : Cᵒᵖ ⥤ C ⥤ Type v`, an object of `ProfunctorElem W` is a triple
`((a, b), w)` where `a : Cᵒᵖ`, `b : C`, and `w : W.obj a |>.obj b`.
-/
abbrev ProfunctorElem (W : Cᵒᵖ ⥤ C ⥤ Type v₇) :=
  (Functor.uncurry.obj W).Elements

/--
The projection functor from the category of elements to `Cᵒᵖ × C`.
-/
abbrev ProfunctorElem.π (W : Cᵒᵖ ⥤ C ⥤ Type v₇) :
    ProfunctorElem W ⥤ Cᵒᵖ × C :=
  CategoryOfElements.π (Functor.uncurry.obj W)

/--
The diagram functor on the category of elements, obtained by composing
the projection with the (uncurried) diagram profunctor.
-/
def profunctorElemDiagram (W : Cᵒᵖ ⥤ C ⥤ Type v₇) (P : Cᵒᵖ ⥤ C ⥤ D) :
    ProfunctorElem W ⥤ D :=
  ProfunctorElem.π W ⋙ Functor.uncurry.obj P

/-- Object-level computation for `profunctorElemDiagram`. -/
@[simp]
theorem profunctorElemDiagram_obj (W : Cᵒᵖ ⥤ C ⥤ Type v₇) (P : Cᵒᵖ ⥤ C ⥤ D)
    (x : ProfunctorElem W) :
    (profunctorElemDiagram W P).obj x = (P.obj x.fst.1).obj x.fst.2 := rfl

/-- Map-level computation for `profunctorElemDiagram`. -/
@[simp]
theorem profunctorElemDiagram_map (W : Cᵒᵖ ⥤ C ⥤ Type v₇) (P : Cᵒᵖ ⥤ C ⥤ D)
    {x y : ProfunctorElem W} (f : x ⟶ y) :
    (profunctorElemDiagram W P).map f = (Functor.uncurry.obj P).map f.val := rfl

/-!
### Cones over the profunctor elements diagram

A cone over `profunctorElemDiagram W P` consists of:
- An apex `pt : D`
- For each `((a, b), w) : ProfunctorElem W`, a morphism `pt ⟶ P(a, b)`
- Natural in `((a, b), w)`: for morphisms `(f, g, h)`, the legs commute

We aim to show this is equivalent to `WeightedWedge W P`.
-/

/--
A cone over the profunctor elements diagram.
-/
abbrev ProfunctorElemCone (W : Cᵒᵖ ⥤ C ⥤ Type v₇) (P : Cᵒᵖ ⥤ C ⥤ D) :=
  Limits.Cone (profunctorElemDiagram W P)

/--
Extract the leg of a `ProfunctorElemCone` at an element.
-/
def ProfunctorElemCone.leg {W : Cᵒᵖ ⥤ C ⥤ Type v₇} {P : Cᵒᵖ ⥤ C ⥤ D}
    (c : ProfunctorElemCone W P) (x : ProfunctorElem W) :
    c.pt ⟶ (P.obj x.fst.1).obj x.fst.2 :=
  c.π.app x

/-!
### Exploring wedge formulations of weighted wedges

The standard wedge-cone correspondence states:
- `Wedge P ≅ Cone (P ∘ π : Tw(C) → D)` for `P : Cᵒᵖ ⥤ C ⥤ D`

We seek a profunctor `Q : Jᵒᵖ ⥤ J ⥤ D` such that `Wedge Q ≅ WeightedWedge W P`.

By the wedge-cone correspondence, this would give:
- `Wedge Q ≅ Cone (Q' : Tw(J) → D)`

For this to match `WeightedWedge W P ≅ Cone (weightedWedgeDiagram W P)`, we need:
- `Tw(J) ≅ (profunctorOnTwistedArrow C W).Elements`
- `Q'` matches `weightedWedgeDiagram W P` under this isomorphism

Let's explore what `J` should be.
-/

/-!
### Analysis: Non-trivial wedge formulations of weighted wedges

We explore whether `WeightedWedge W P` can be expressed as an ordinary
`Wedge Q` for some non-trivial profunctor `Q`.

The standard wedge-cone correspondence states:
```
Wedge Q ≅ Cone (Q ∘ π : Tw(J) → D)
```
for a profunctor `Q : Jᵒᵖ × J → D`, where `π : Tw(J) → Jᵒᵖ × J`.

We already have:
```
WeightedWedge W P ≅ Cone (weightedWedgeDiagram W P : El(W') → D)
```
where `W' = profunctorOnTwistedArrow C W`.

For `WeightedWedge W P ≅ Wedge Q` with non-trivial `Q`, we would need a
category `J` such that:
1. `Tw(J) ≅ El(W')`
2. Under this isomorphism, `Q ∘ π` matches `weightedWedgeDiagram W P`

The first condition requires `J` such that morphisms of `J` (as objects of
`Tw(J)`) correspond to pairs `(tw, w)` where `tw` is a twisted arrow and
`w : W(twDom tw, twCod tw)` is a weight.

This is a strong constraint. For general `W`, no such `J` exists because:
- `El(W')` can have objects at arbitrary pairs `(c, d)` with weight
- `Tw(J)` objects are morphisms `f : a → b` in `J`
- The latter has additional structure (source and target objects in `J`)

For the special case where `W = Hom_C` (the hom profunctor):
- `El(Hom_C ∘ forget) ≅ Tw(C)` (each `(tw, w)` corresponds to `tw` itself)
- `DiagElem(Hom_C) ≅ C` (diagonal elements are identity morphisms)
- `Tw(DiagElem(Hom_C)) ≅ Tw(C)`

So for `W = Hom_C`, we recover the standard wedge-cone correspondence.

For general `W`, the trivial wedge formulation via `wedgeConstProfEquivCone`
(using a constant profunctor that ignores the contravariant slot) remains
the most direct formulation.

The "non-trivial" profunctor `Q((tw₁, w₁), (tw₂, w₂)) = P(tw₁.dom, tw₂.cod)`
gives the right diagonal values but produces a different condition:
- Wedge condition: dinaturality `Q(1,f) ∘ ω_x = Q(f,1) ∘ ω_y`
- Weighted wedge condition: naturality `diagram(f) ∘ leg_x = leg_y`

These coincide only when `Tw(J) ≅ J`, which happens when `J` is a groupoid.
-/

end WeightedWedgeElementsEquivalence
