...
section ProjProfDialgebraEquiv

/-!
### Projection profunctor dialgebra equivalence

For functors `F, G : C ⥤ Type v`, the profunctor-dialgebra profunctor
`ProfDialgebraProf (ProjProf C F) (ProjProf C G)` is definitionally equal to
the dialgebra profunctor `DialgebraProf F G`.

This allows composing the hexagon-DiagElem equivalence with the
DiagElem-Dialgebra equivalence.

Note: The functors `F, G : C ⥤ Type v` must map into `Type v` (the morphism
universe of `C`) because `HexagonObj P Q` requires profunctors
`P Q : Cᵒᵖ × C ⥤ Type v`.
-/
...
/-- The dialgebra profunctor specialized to `Type v` target.
This is `DialgebraProf F G` where `F G : C ⥤ Type v`. -/
abbrev DialgebraProfType : Cᵒᵖ ⥤ C ⥤ Type v where
  obj x := {
  ...
  map {x₁ x₂} f := {
    ...
    naturality := fun _ _ g => by ext; simp
  }
  map_id x := by ext y a; simp
  map_comp {x₁ x₂ x₃} f g := by ext y a; simp

/-- The dialgebra category specialized to `Type v` target. -/
abbrev DialgebraType := @Dialgebra C _ (Type v) _ F G

/-- `ProfDialgebraProf (ProjProf C F) (ProjProf C G)` and `DialgebraProfType F G`
have equal object maps at each point. -/
theorem profDialgebraProjProf_obj_eq (a : Cᵒᵖ) (b : C) :
    ((ProfDialgebraProf (ProjProf C F) (ProjProf C G)).obj a).obj b =
    ((DialgebraProfType F G).obj a).obj b := rfl

/-- `ProfDialgebraProf (ProjProf C F) (ProjProf C G)` equals `DialgebraProfType F G`
as profunctors. -/
theorem profDialgebraProjProf_eq :
    ProfDialgebraProf (ProjProf C F) (ProjProf C G) = DialgebraProfType F G := by
  refine Functor.ext ?h_obj ?h_map
  · intro a
    refine Functor.ext ?h_obj.h_obj ?h_obj.h_map
    · intro b; rfl
    · intro b₁ b₂ g
      simp only [eqToHom_refl, Category.comp_id, Category.id_comp]
      ext φ
      funext p
      simp only [ProfDialgebraProf_obj_map, ProjProf_map₁, ProjProf_map₂]
      rfl
  · intro a₁ a₂ f
    ext b φ
    funext p
    simp only [ProfDialgebraProf_map_app, ProjProf_map₁, ProjProf_map₂,
      NatTrans.comp_app, types_comp_apply]
    simp only [eqToHom_app, types_comp_apply, eqToHom_refl, types_id_apply]

/-- `DialgebraProfType F G` equals `DialgebraProf F G` from ParanatAlg.lean. -/
theorem dialgebraProfType_eq_dialgebraProf :
    DialgebraProfType F G = DialgebraProf F G := by
  refine Functor.ext ?h_obj ?h_map
  · intro a
    refine Functor.ext ?h_obj.h_obj ?h_obj.h_map
    · intro b; rfl
...
/-- The projection profunctor for F, specialized to C. -/
abbrev ProjProfF : Cᵒᵖ × C ⥤ Type v := @ProjProf C _ C _ F
