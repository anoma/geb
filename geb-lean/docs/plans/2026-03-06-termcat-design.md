...
### The Omega_2 Boundary and Primitive Recursion

The `Omega_2` subtyping axiom
(`Omega_2 < Omega_2 -> Fix (forall X_vec. U -> V)`) is
the sole axiom that enables typing of recursive functions.
All recursion in tree calculus goes through `Z{f}` (or
`Y_2{f}`), which is built from `omega_2`. Without the
`Omega_2` axiom, no recursive functions can be typed —
not even primitive-recursive ones like fold over trees.

This means that Jay's type system without `Omega_2` does
NOT characterize the primitive-recursive fragment. It
characterizes something different: the non-recursive
combinators (K, S, I, triage, wait, swap, etc.) plus
iteration over Church-encoded data whose fold is built
into their quantified type (e.g.,
`Nat = forall X. (X -> X) -> (X -> X)` has iteration
built in because a Church numeral `n` applies a function
`n` times).

Fold over natural binary trees — the catamorphism
(`polyFixFold`) that defines our primitive-recursive
fragment — falls on the wrong side of the `Omega_2`
boundary. It must be implemented via `Z{f}` in tree
calculus, so it requires `Omega_2` even though it is
guaranteed to terminate.

This creates a mismatch between our polynomial
infrastructure and Jay's type system:

- In the polynomial setting, fold (`polyFixFold`) is
  the universal property of the initial algebra. Its
  termination is guaranteed by the well-foundedness of
  the W-type. No general fixpoint is involved.
- In tree calculus, fold must be *programmed* using
  general recursion (`Z{f}`), and the type system
  cannot distinguish this terminating use of `Z{f}`
  from a non-terminating one.

Several approaches could bridge this gap:

1. **Add a fold combinator as a primitive.** Extend tree
   calculus with a built-in fold operator and a
   corresponding subtyping axiom
   (e.g., `Fold < (L -> R) -> (R -> R -> R) -> T -> R`
   where `T` is the tree type). This gives fold its own
   typing rule independent of `Omega_2`, at the cost of
   departing from pure tree calculus.

2. **Use Church-encoded trees.** Define
   `Tree_C = forall X. X -> (X -> X -> X) -> X` as a
   Church-encoded tree type with built-in fold. Programs
   of this type carry their fold with them. The
   subtyping `forall X. T < {U/X} T` instantiates the
   fold. This stays within Jay's type system but uses a
   different representation of trees than the natural
   binary trees that triage operates on.

3. **Use a sized-type or well-founded recursion
   discipline.** Replace the `Omega_2` axiom with a
   family of axioms indexed by size bounds, allowing
   `Z{f}` to be typed when the recursion is bounded.
   This is more expressive than Option 1 but requires a
   notion of "size" in the type system.

4. **Accept the mismatch.** Use the syntactic
   characterization (absence of `omega_2` subpatterns)
   for the primitive-recursive self-recognizer, and
   treat Jay's type system as relevant to the full
   language rather than the restricted fragment.

Option 4 is the path of least resistance for Phase 2.
Options 1-3 become relevant in Phase 4 (language tower
and proof-carrying code), where a type-theoretic
characterization of terminating programs would be
valuable.

### Implications for the Self-Recognizer

The self-recognizer needs to decide whether a given tree
is in the primitive-recursive fragment. Given the
mismatch above, Jay's type system does not directly
provide the characterization. The recognizer should use
the syntactic approach:

- **Syntactic recognizer**: inspect the tree structure
  for the absence of fixpoint combinators (`omega_2`
  or patterns that reduce to general fixpoints). This
  is a pattern-matching task on trees, directly
  expressible using triage. A term is in the
  primitive-recursive fragment if it uses triage for
  case analysis and structural recursion (fold) but
  does not contain subterms equivalent to `omega_2`.

The type-checking approach (implement a type-checker
for a restricted type system) remains relevant as a
later step, but requires resolving the mismatch
(Options 1-3 above) to distinguish terminating from
non-terminating uses of `Z{f}`.
...
- **Layer separation**: the language tower could be
  ...
  available. However, as noted in the section on the
  `Omega_2` boundary, simply removing `Omega_2` does
  not yield the primitive-recursive fragment (it yields
  something weaker). A layer for primitive recursion
  over trees would require additional infrastructure
  (a fold primitive, sized types, or Church-encoded
  trees) beyond what the `Omega_2`-free system
  provides. Layers above that could add `Omega_2` for
  general recursion, `A` types, covariance axioms,
  etc.
...
- **The full type system may inform later phases.**
  Although the type system does not directly
  characterize the primitive-recursive fragment (see
  the section on the `Omega_2` boundary above), type
  inference and type-checking for the full system are
  relevant to the language tower (Phase 4), where
  programs will carry types.
- **Type-checking vs type inference.** For the
  self-recognizer (Phase 2), we need a syntactic check,
  not a type-checker. For later phases, we need
  type-*checking* (given a term and a type, verify the
  derivation), not type *inference* (given a term, find
  a type). Type-checking is typically easier than
  inference and may be feasible even when inference is
  not.
- **Type-checking as a tree-calculus program.** The
  ...
  for implementation as tree-calculus programs (they
  are fold-definable over the type and term trees,
  and fold over trees is expressible via `Z{f}` in
  tree calculus).
...
- **Bridging the Omega_2 gap**: can a fold combinator,
  sized types, or a well-founded recursion discipline
  (Options 1-3 above) provide a type-theoretic
  characterization of the primitive-recursive fragment
  that Jay's unmodified system lacks?

- **Church-encoded vs natural trees**: what is the
  precise relationship between programs operating on
  natural binary trees (via triage) and programs
  operating on Church-encoded trees (via the built-in
  fold of a quantified type)? Can a translation between
  the two representations be typed?
