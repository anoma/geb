...
- Arbitrary infinite free algebras, including the tree-algebra
  ...
  `1 + X`; composing with this workstream's `1 + X` to
  `LawvereERCat` equivalence then yields the ER characterization
  for every algebra, with all machine involvement confined to the
  `1 + X` case. The two directions are definability transports
  across a coding bijection between the algebra and `N`:
  (i) the unary sub-algebra of an infinite algebra - generated by a
  0-ary and a positive-arity constructor, the alpha/beta device of
  the paper's section 2.4(5) - carries a theory morphism from the
  `1 + X` system; (ii) encoding and decoding between the algebra
  and its unary sub-algebra are built by ramified structural
  recursion from pairing functions, whose arithmetic lives in the
  free algebra of `1 + X` (the repository's Szudzik machinery,
  `GebLean/Utilities/Tupling.lean`, on the ER side), with
  elementary code growth. With interpretative hom-sets the functor
  laws are extensional, so the packaging is the `erKSimEquiv`
  pattern. A design point for that workstream: state the
  ...
  (Lemma 6, spelled out) and never compares two ramified systems
  ...
  register machines for the given algebra. The future workstream
  chooses between the two and marks accordingly.
