...
- [x] **Step 1: define the type.** Base-2 normal form (no `expBase`
...
- [x] **Step 3: `ZMonomial.evalNat`** (the unsigned ℕ magnitude) and
...
- [x] **Step 4: safeguard docstring.** Record in the module docstring:
...
- [x] **Step 6: commit.** `feat(era): add the ZMonomial signed reflection type`
...
- [x] **Step 1: define the product.** Sign XOR, coefficient product,
...
- [x] **Step 2: state the agreement lemma.**
...
- [x] **Step 3: prove it.** Strategy: unfold `eval`/`mul`; the sign
...
- [x] **Step 4: numeric probe (ℕ, scratch).** Pick a tiny `m`, concrete
  `ρ`, two monomials; check `(a.mul b).evalNat ρ = a.evalNat ρ *
  b.evalNat ρ` with `#eval`. Delete before commit.

- [x] **Step 5: build + lint + axioms** (commands as A1 Step 5).

- [x] **Step 6: commit.** `feat(era): define ZMonomial product with eval agreement`
...
- [x] **Step 1: the coeff grammar predicate.** A monomial coeff is a
...
- [x] **Step 2: the extraction.** Structural recursion over an
...
- [x] **Step 3: `diophOf` coeffs satisfy the grammar.** Mirror
...
- [x] **Step 4: build + lint + axioms.**

- [x] **Step 5: commit.** `feat(era): extract cube-variable degree from coeff`
...
- [x] **Step 1: lift a single monomial (degree into `polyExp`).** Run
...
- [x] **Step 3: lift a sum.** `SimpleSum.toZ : SimpleSum (p+k) → List
...
- [x] **Step 5: commit.** `feat(era): reflect SimpleMonomial sums into ZMonomial`
...
- [x] **Step 1: define the atom reflection.** For `sqDist P Q`, produce
...
- [x] **Step 2: the pointwise agreement on values.**
...
- [x] **Step 3: discharge the hypotheses from `diophOf`.** Apply
...
- [x] **Step 5: commit.** `feat(era): reflect the SoS predicate value over ZMonomial`
...
- [x] **Step 1: chain equations.** For a cube variable `x` of
...
- [x] **Step 2: substitution.** Replace each non-exponential occurrence
...
- [x] **Step 3: build + lint + axioms.**

- [x] **Step 4: commit.** `feat(era): build Lemma 3.5 chain variables and substitution`
...
- [x] **Step 1: define the reduced system.** `R := ∑ Sᵢ² + P_sub²`
...
- [x] **Step 2: state and prove the degree bound.** Every monomial of
...
- [x] **Step 4: commit.** `feat(era): bound the reduced system to degree 2`
...
- [x] **Step 1: state it** (paper Lemma 3.5, condition 2).
...
- [x] **Step 2: prove it.** `R = 0` is a sum of squares over ℤ, so each
...
- [x] **Step 4: commit.** `feat(era): prove Lemma 3.5 soundness of the reduced system`
...
- [x] **Step 1: state it** (paper Lemma 3.5, condition 3).
...
- [x] **Step 2: prove it.** The chain equations force `y_i = x^i`
...
- [x] **Step 4: commit.** `feat(era): prove Lemma 3.5 unique witness`
...
- [x] **Step 1: split parameter vs cube slots.** For a degree-≤2
...
- [x] **Step 2: build + lint + axioms.**

- [x] **Step 3: commit.** `feat(era): put degree-2 ZMonomials in cube form`
...
- [x] **Step 1: the weighted cube sum of one monomial.** With the
...
- [x] **Step 2: realise each `G_u` as a term and discharge base ≥ 2.**
...
- [x] **Step 4: commit.** `feat(era): factor a degree-2 monomial cube sum`
...
- [x] **Step 1: define and evaluate.** `ε` is the parameter-only
...
- [x] **Step 3: commit.** `feat(era): realise the Eq (7) constant part as a term`
...
- [x] **Step 1: assemble `packM_term`** (spec Step 4, pinned sign
...
- [x] **Step 2: the master eval lemma.**
...
- [x] **Step 3: numeric probe (ℕ, scratch).** Tiny `s`, `k`, concrete
  `w`, `t`, `ctx`; check `Tm.eval … (packM_term …) ctx` equals
  `packM …` by `#eval` on ℕ. Delete before commit.

- [x] **Step 4: build + lint + axioms.**

- [x] **Step 5: commit.** `feat(era): assemble the packed-witness term via Eqs (7),(8)`
...
- [x] **Step 1: define `eraCount`** by feeding `packM_term` of the
...
- [x] **Step 2: `eraCount_eval` for the reduced system** (count of its
...
- [x] **Step 4: commit.** `feat(era): count a reduced system's cube zeros as a term`
...
- [x] **Step 1: build `θ`.** The chain variables satisfy `y_i ≤ x^i`;
...
- [x] **Step 2: build the enlarged `w`.** `eraW` dominates
...
- [x] **Step 4: commit.** `feat(era): exhibit the Lemma 3.5 cube bounds as Era terms`
...
- [x] **Step 1: fibre collapse at the enlarged side.** For each
...
- [x] **Step 2: shell-empty bridge `tθ → t`.** The first draft left a
...
- [x] **Step 3: compose with `diophOf_unique`.** When `s = (diophOf
...
- [x] **Step 5: commit.** `feat(era): collapse enlarged-cube count to base`
...
- [x] **Step 1: reconcile the `diophOf` index layout.** `diophOf τ` for
...
- [x] **Step 2: state the public combinator** that 6.4d/6.4e call:
...
- [x] **Step 4: commit.** `feat(era): expose the Era count combinator`

---

## Self-review checklist (run before adversarial review)

- [x] **Spec coverage.** Step 1 → Phase A; Step 2 → Phase B; Step 3 →
  Phase C; Step 4 → Phase D; Step 5 → Phase E. Each spec obligation maps
  to a task.
- [x] **No placeholders.** Every code step shows a signature; every
