...
`GebLeanDocs` is a Verso manual for the ramified-recurrence area
(tutorial and reference parts sampling the declarations
summarised under [Ramified recurrence and the elementary
characterization](#ramified-recurrence-and-the-elementary-characterization)
above), generated by `lake exe geblean-docs`. It is built and
linted only in CI, not by `lake build`, `lake test`, or
`lake lint` locally; see `.claude/rules/ci-and-workflow.md` §
Pre-commit checklist. See also
[`areas/ramified-recurrence.md`](areas/ramified-recurrence.md) §
Pointers.
